...
exports.pages = async (req, res, next) => {
  ...
  let pageData = {};
  await globalModel
    .custom(req, 'SELECT * from pages WHERE url = ?', [id])
    .then(results => {
      if (results) {
...
exports.index = async (req, res, next) => {
  ...
  await slideshow.findAll(req, { enabled: 1 }).then(result => {
    if (result && result.length > 0) req.query.slideshow = result;
  });
  ...
  if (req.appSettings['video_tip'] == 1) {
    ...
    if (!cachedData) {
      await videoModel
        .donorsOfTheMonth({ limit: 10, offthemonth: true }, req)
        .then(result => {
  ...
  if (req.appSettings['video_featured'] == 1) {
    ...
    if (!cachedData) {
      await videoModel
        .getVideos(req, { is_featured: 1, limit: 8, orderby: 'random' })
        .then(result => {
          if (result && result.length > 0) {
  ...
  if (req.appSettings['video_sponsored'] == 1) {
    ...
    if (!cachedData) {
      await videoModel
        .getVideos(req, { is_sponsored: 1, limit: 8, orderby: 'random' })
        .then(result => {
          if (result && result.length > 0) {
  ...
  if (req.appSettings['video_hot'] == 1) {
    ...
    if (!cachedData) {
      await videoModel
        .getVideos(req, { is_hot: 1, limit: 8, orderby: 'random' })
        .then(result => {
          if (result && result.length > 0) {
  ...
  if (req.appSettings['enable_audio'] == 1) {
    ...
    if (!cachedData) {
      await audioModel
        .getAudios(req, { limit: 10, orderby: 'random' })
        .then(result => {
          if (result && result.length > 0) {
  ...
  if (req.appSettings['enable_channel'] == 1) {
    ...
    if (req.appSettings['channel_featured'] == 1) {
      ...
      if (!cachedData) {
        await channelModel
          .getChannels(req, { is_featured: 1, limit: 4, orderby: 'random' })
          .then(result => {
            if (result && result.length > 0) {
    ...
    if (req.appSettings['channel_sponsored'] == 1) {
      ...
      if (!cachedData) {
        await channelModel
          .getChannels(req, { is_sponsored: 1, limit: 4, orderby: 'random' })
          .then(result => {
            if (result && result.length > 0) {
    ...
    if (req.appSettings['channel_hot'] == 1) {
      ...
      if (!cachedData) {
        await channelModel
          .getChannels(req, { is_hot: 1, limit: 4, orderby: 'random' })
          .then(result => {
            if (result && result.length > 0) {
  ...
  if (req.query.videoCategories && req.query.videoCategories.length > 0) {
    ...
    if (!cachedCategoriesVideos) {
      ...
      myCache.set(
        'home_categories_videos',
        req.query.categoryVideos,
        req.ttlTime,
      );
    } else {
...
exports.categories = async (req, results) => {
  ...
  return new Promise(function (resolve, reject) {
    async.forEachOf(
      results,
      async function (category, i, callback) {
        const data = {};
...
exports.cronFunction = async (req, res) => {
  return new Promise(function (resolve, reject) {
    req.getConnection(function (err, connection) {
      console.log('get start chron function');
      connection.query(
        'SELECT * from tasks WHERE started = 0 AND (start_time IS NULL || DATE_ADD(start_time, INTERVAL `timeout` second) <= ? ) ORDER BY priority ASC',
        [dateTime.create().format('Y-m-d H:M:S')],
        async function (err, results, fields) {
          console.log('results: FOR TASKS', results);
          if (!err) {
            await asyncForEach(results, async (notification, i) => {
              ...
              if (taskResult.type == 'notifications') {
                await notificationModel
                  .findAll(req, { limit: 200, notification_send: 2 })
                  .then(async result => {
                    //set cron started
                    connection.query(
                      'UPDATE tasks SET started = 1,start_time = ? WHERE type = ?',
                      [
                        dateTime.create().format('Y-m-d H:M:S'),
                        taskResult.type,
                      ],
                    ...
                    if (result && result.length > 0) {
                      await asyncForEach(result, async (data, i) => {
                        await notificationModel
                          .getNotification(req, data)
                          .then(async result => {
                            if (result) {
                              globalModel
                                .custom(
                                  req,
                                  'UPDATE notifications SET notification_send = 1 WHERE notification_id = ?',
                                ...
                                .custom(
                                  ...
                                  'SELECT ' +
                                    ...
                                    (column == 'user_id'
                                      ? 'user_id'
                                      : 'owner_id') +
                                    ',emailsettings.type FROM ' +
                                    result.object_type +
                                    ' LEFT JOIN emailsettings ON emailsettings.owner_id = ' +
                                    result.object_type +
                                    '.' +
                                    (column == 'user_id'
                                      ? 'user_id'
                                      : 'owner_id') +
                                    ' AND emailsettings.type = "' +
                                    result.type +
                                    '" AND emailsettings.email = 0 WHERE ' +
                                    column +
                                    ' = ? ',
                                  [result.object_id],
                                )
                                      ...
                                      .then(async ownerData => {
                                        if (ownerData) {
                                          await globalModel
                                            .custom(
                                              req,
                                              'SELECT vars,type FROM emailtemplates WHERE type = ?',
                                              [result.type],
                                            )
                                            .then(async resultsType => {
                                              if (resultsType) {
                                                const typeData = JSON.parse(
                                                  JSON.stringify(resultsType),
                                                )[0];
                                                ...
                                                result.ownerEmail =
                                                  ownerData[0];
                                                result.toName =
                                                  ownerData[0].displayname;
                                                result.toEmail =
                                                  ownerData[0].email;
                                                const i18n = require('i18next');
                                                i18n.changeLanguage(
                                                  ownerData[0].language,
                                                );
                                                req.i18n = i18n;
                                                await emailFunction
                                                  .sendMessage(
                    ...
                    connection.query(
                      'UPDATE tasks SET started = 0 WHERE type = ?',
                      [taskResult.type],
                      async function (err, results, fields) {},
                    );
                  });
              ...
              if (taskResult.type == 'videoEncode') {
                connection.query(
                  "SELECT * from videos WHERE completed = 0 AND type = 3 AND custom_url != '' LIMIT 1",
                  [],
                  async function (err, results, fields) {
                    if (!err) {
                      ...
                      if (videos.length) {
                        ...
                        await videoController
                          ...
                          .then(result => {
                            ...
                            console.log('Video Got Converted');
                          });
                        //});
                      }
                    }
                    ...
                    await asyncForEach(
                      ...
                      async (subscription, i) => {
                        ...
                        globalModel
                          .custom(
                            req,
                            'UPDATE `subscriptions` SET `status` = ? , `is_level_change` = 1 WHERE subscription_id = ?',
                            ['expired', subscription.subscription_id],
                          )
                          .then(res => {})
                          .catch(err => {
                            console.log('ERROR IN LEVEL CHAGE CRON ', err);
              ...
              if (taskResult.type == 'userExpiryNotifications') {
                ...
                let currenttime = dateTime.create().format('Y-m-d H:M:S');
                let sql =
                  "SELECT `subscriptions`.*,packages.*,userdetails.*,users.email FROM `subscriptions`  JOIN `users` ON users.user_id =subscriptions.owner_id JOIN `userdetails` ON users.user_id =userdetails.user_id  LEFT JOIN `packages` ON packages.package_id = subscriptions.package_id WHERE (CASE WHEN expiration_date IS NULL then false WHEN expiration_date IS NOT NULL AND packages.alert_type = 'minutes' THEN DATE_SUB(expiration_date,INTERVAL `alert_number` MINUTE) <= '" +
                  currenttime +
                  "' WHEN expiration_date IS NOT NULL AND packages.alert_type = 'hours' THEN DATE_SUB(expiration_date,INTERVAL `alert_number` HOUR) <= '" +
                  currenttime +
                  "' WHEN expiration_date IS NOT NULL AND packages.alert_type = 'days' THEN DATE_SUB(expiration_date,INTERVAL `alert_number` DAY) <= '" +
                  currenttime +
                  "' WHEN expiration_date IS NOT NULL AND packages.alert_type = 'weeks' THEN DATE_SUB(expiration_date,INTERVAL `alert_number` WEEK) <= '" +
                  currenttime +
                  "' WHEN expiration_date IS NULL AND packages.`alert_type` = 'minutes' THEN DATE_SUB(subscriptions.creation_date,INTERVAL `alert_number` MINUTE) <= '" +
                  currenttime +
                  "' WHEN expiration_date IS NULL AND packages.`alert_type` = 'hours' THEN DATE_SUB(subscriptions.creation_date,INTERVAL `alert_number` HOUR) <= '" +
                  currenttime +
                  "' WHEN expiration_date IS NULL AND packages.`alert_type` = 'days' THEN DATE_SUB(subscriptions.creation_date,INTERVAL `alert_number` DAY) <= '" +
                  currenttime +
                  "' ELSE DATE_SUB(subscriptions.creation_date,INTERVAL `alert_number` WEEK) <= '" +
                  currenttime +
                  "' END ) AND is_notification_send = 0 AND (subscriptions.status = 'active' || subscriptions.status = 'completed' || subscriptions.status = 'approved') AND (users.user_id IS NOT NULL) AND subscriptions.type = 'member_subscription'  ORDER BY `subscriptions`.`expiration_date` ASC LIMIT 100";
                await globalModel.custom(req, sql).then(async results => {
                  let subscriptions = JSON.parse(JSON.stringify(results));
                  if (subscriptions.length) {
                    await asyncForEach(
                      ...
                      async (subscription, i) => {
                        ...
                        if (subscription.email_notification == 1) {
                          connection.query(
                            'SELECT vars,type FROM emailtemplates WHERE type = ?',
                            ['level_member_expiry'],
                            function (err, resultsType, fields) {
                              if (!err) {
                                const typeData = JSON.parse(
                                  JSON.stringify(resultsType),
                                )[0];
                                ...
                                result['planName'] = {};
                                result['planName']['title'] =
                                  subscription.title;
                                result['planName']['type'] = 'text';
                                result['period'] = {};
                                result['period']['title'] =
                                  subscription.alert_number +
                                  ' ' +
                          ...
                          connection.query(
                            ...
                            function (err, results, fields) {
                              notificationData.notification_id =
                                results.insertId;
                              notificationModel
                                .getNotification(req, notificationData)
                                .then(async result => {
                                  socketio.getIO().emit('notifications', {
                                    owner_id: result.owner_id,
                                    notification: result,
                        ...
                        globalModel
                          .custom(
                            req,
                            'UPDATE `subscriptions` SET `is_notification_send` = 1 WHERE subscription_id = ?',
                ...
                await connection.query(
                  "SELECT * from videos WHERE (custom_url IS NULL || custom_url = '') AND type = 3 AND creation_date < ? LIMIT 20",
                  [dateTimePrevious],
                  async function (err, results, fields) {
                    if (!err) {
                      let videos = JSON.parse(JSON.stringify(results));
...
exports.manifest = async (req, res) => {
  ...
  data['app_icon'] =
    req.appSettings['imageSuffix'] + req.appSettings['pwa_icon_sizes_512'];
  data['start_url'] = process.env.PUBLIC_URL;

  let icons = [];
  icons.push({
    src: req.appSettings['imageSuffix'] + req.appSettings['pwa_icon_sizes_72'],
    sizes: '72x72',
    type: 'image/png',
  });
  icons.push({
    src: req.appSettings['imageSuffix'] + req.appSettings['pwa_icon_sizes_96'],
    sizes: '96x96',
    type: 'image/png',
  });
  icons.push({
    src: req.appSettings['imageSuffix'] + req.appSettings['pwa_icon_sizes_128'],
    sizes: '128x128',
    type: 'image/png',
  });
  icons.push({
    src: req.appSettings['imageSuffix'] + req.appSettings['pwa_icon_sizes_144'],
    sizes: '144x144',
    type: 'image/png',
  });
  icons.push({
    src: req.appSettings['imageSuffix'] + req.appSettings['pwa_icon_sizes_152'],
    sizes: '152x152',
    type: 'image/png',
  });
  icons.push({
    src: req.appSettings['imageSuffix'] + req.appSettings['pwa_icon_sizes_192'],
    sizes: '192x192',
    type: 'image/png',
  });
  icons.push({
    src: req.appSettings['imageSuffix'] + req.appSettings['pwa_icon_sizes_384'],
    sizes: '384x384',
    type: 'image/png',
  });
  icons.push({
    src: req.appSettings['imageSuffix'] + req.appSettings['pwa_icon_sizes_512'],
    sizes: '512x512',
    type: 'image/png',
  });
  data.icons = icons;
...
exports.sitemap = async (req, res) => {
  ...
  await new Promise(function (resolve, reject) {
    req.getConnection(function (err, connection) {
      connection.query(
        "SELECT username as custom_url,displayname as title,'users' as item,users.modified_date from users LEFT JOIN userdetails on users.user_id = userdetails.user_id WHERE active = 1 AND approve = 1 AND userdetails.search = 1",
        [],
        function (err, results, fields) {
          if (err) resolve();
          if (results) {
  ...
  await new Promise(function (resolve, reject) {
    req.getConnection(function (err, connection) {
      connection.query(
        "SELECT custom_url,title,'videos' as item,modified_date from videos WHERE approve = 1 AND completed = 1 AND ( is_livestreaming = 0 AND (code IS NOT NULL || videos.type = 3) )  AND is_locked = 0  AND search = 1 AND (view_privacy= 'everyone' || view_privacy IS NULL) ",
        [],
        function (err, results, fields) {
          if (err) resolve();
          if (results) {
  ...
  if (req.appSettings['enable_blog'] == 1) {
    ...
    await new Promise(function (resolve, reject) {
      req.getConnection(function (err, connection) {
        connection.query(
          "SELECT custom_url,title,'blogs' as item,modified_date from blogs WHERE approve = 1 AND draft = 1 AND search = 1 AND (view_privacy= 'everyone' || view_privacy IS NULL) ",
          [],
          function (err, results, fields) {
            if (err) resolve();
            if (results) {
  ...
  if (req.appSettings['enable_channel'] == 1) {
    ...
    await new Promise(function (resolve, reject) {
      req.getConnection(function (err, connection) {
        connection.query(
          "SELECT custom_url,title,'channels' as item,modified_date from channels WHERE approve = 1 AND search = 1 AND (view_privacy= 'everyone' || view_privacy IS NULL) AND is_locked = 0 ",
          [],
          function (err, results, fields) {
            if (err) resolve();
            if (results) {
