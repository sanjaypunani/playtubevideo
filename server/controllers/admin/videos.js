...
exports.index = async (req, res) => {
  ...
  let sql =
    'SELECT COUNT(*) as totalCount FROM videos LEFT JOIN users on users.user_id = videos.owner_id  LEFT JOIN userdetails ON users.user_id = userdetails.user_id WHERE 1 = 1 AND (custom_url IS NOT NULL) ' +
    conditionalWhere;
  ...
  if (totalCount > 0) {
    ...
    let sqlQuery =
      'SELECT videos.*,userdetails.username,userdetails.displayname FROM videos LEFT JOIN users on users.user_id = videos.owner_id  LEFT JOIN userdetails ON users.user_id = userdetails.user_id  WHERE 1 = 1 AND (custom_url IS NOT NULL) ' +
      conditionalWhere;
  ...
  res.render('admin/videos/index', {
    ...
    loggedin_id: req.user ? req.user.user_id : '',
    loggedinLevel_id: req.user ? req.user.level_id : '',
  ...
  await globalModel
    ...
    .then(async result => {
      if (result && result.length) {
        let item = result[0];
        await globalModel
          .update(req, { approve: !item.approve }, 'videos', 'video_id', id)
          .then(result => {
            if (item.owner_id != req.user.user_id && !item.approve) {
              notifications
                .insert(req, {
                  owner_id: item.owner_id,
                  type: 'videos_admin_approved',
                  subject_type: 'users',
  ...
  await globalModel
    ...
    .then(async result => {
      if (result && result.length) {
        let item = result[0];
        await globalModel
          .update(req, { is_hot: !item.is_hot }, 'videos', 'video_id', id)
          .then(result => {
  ...
  var reg_form = forms.create(
    {
      ...
      file: fields.string({
        ...
        cssClasses: { field: ['form-group'] },
        widget: formFunctions.file({
          name: 'file',
          value: existingArtist.image ? imageSuffix + existingArtist.image : '',
        }),
      }),
    },
  ...
  reg_form.handle(req, {
    success: function (form) {
      ...
      res.send({
        ...
        url: process.env.ADMIN_SLUG + '/videos/artists',
      });
    },
    error: function (form) {
    ...
    other: function (form) {
      res.render('admin/videos/artists/create', {
        nav: url,
        reg_form: reg_form,
        title: (!artist_id ? 'Add' : 'Edit') + ' Video Artist',
      });
    },
  });
};
...
exports.getGalleries = async (req, res, next) => {
  ...
  let artists = {};
  await globalModel
    .custom(req, 'SELECT * FROM artists where artist_id = ?', [artist_id])
    .then(result => {
      if (result && result[0]) artists = result[0];
    });
  ...
  let sql =
    'SELECT COUNT(*) as totalCount FROM artist_photos ' + conditionalWhere;
  await globalModel.custom(req, sql, condition).then(result => {
    totalCount = result[0].totalCount;
  });
  ...
  if (totalCount > 0) {
    ...
    condition.push((page - 1) * LimitNum);
    conditionalWhere +=
      ' ORDER BY artist_photos.photo_id DESC limit ? offset ?';
    let sqlQuery =
      'SELECT artist_photos.* FROM artist_photos ' + conditionalWhere;
    await globalModel.custom(req, sqlQuery, condition).then(result => {
      results = result;
    });
  }

  const paggingData = pagging.create(
    req,
    totalCount,
    page,
    '',
  ...
  const url = req.originalUrl.replace(process.env.ADMIN_SLUG, '');
  res.render('admin/videos/artists/gallery', {
    loggedin_id: req.user ? req.user.user_id : '',
    artists: artists,
    totalCount: totalCount,
    query: query,
    nav: url,
    results: results,
    title: 'Manage ' + artists.title + ' Artist Gallery',
    paggingData: paggingData,
  });
};
exports.createGallery = async (req, res) => {
  ...
  if (photo_id) {
    await globalModel
      .custom(req, 'SELECT * FROM artist_photos where photo_id = ?', [photo_id])
      .then(result => {
        if (result && result[0]) existingPhoto = result[0];
      });
  } else {
    if (!req.fileName && req.checkImage) {
      res.send({ errors: { file: 'Please select file.' } });
  ...
  var reg_form = forms.create(
    {
      ...
      file: fields.string({
        ...
        widget: formFunctions.file({
          name: 'file',
          value: existingPhoto.image ? imageSuffix + existingPhoto.image : '',
        }),
      }),
    },
    { validatePastFirstError: true },
  ...
  reg_form.handle(req, {
    ...
    other: function (form) {
      res.render('admin/videos/artists/createphoto', {
        nav: url,
        reg_form: reg_form,
        title:
          (!artist_id ? 'Add ' : 'Edit ') + artists.title + ' Artist Photo',
      });
    },
  });
};
exports.deleteGallery = async (req, res) => {
  ...
  await globalModel
    .custom(req, 'DELETE FROM artist_photos WHERE photo_id = ?', [id])
    .then(result => {
      res.redirect(backURL);
      return;
    });
...
exports.soldVideos = async (req, res) => {
  ...
  let sql =
    "SELECT COUNT(*) as totalCount FROM transactions INNER JOIN videos on videos.video_id = transactions.id LEFT JOIN users on users.user_id = transactions.owner_id  INNER JOIN userdetails ON users.user_id = userdetails.user_id WHERE 1 = 1 AND users.active = '1' AND users.approve = '1' AND (transactions.state = 'approved' || transactions.state = 'completed' || transactions.state = 'active') AND transactions.type = 'video_purchase' " +
    conditionalWhere;
  await globalModel.custom(req, sql, condition).then(result => {
    totalCount = result[0].totalCount;
  });
  ...
  if (totalCount > 0) {
    ...
    let sqlQuery =
      "SELECT transactions.*,userdetails.username,userdetails.displayname,videos.title as videoTitle,videos.custom_url as video_url,transactions.price as amount FROM transactions INNER JOIN videos on videos.video_id = transactions.id INNER JOIN users on users.user_id = transactions.owner_id  INNER JOIN userdetails ON users.user_id = userdetails.user_id  WHERE 1 = 1 AND users.active = '1' AND users.approve = '1' AND (transactions.state = 'approved' || transactions.state = 'completed' || transactions.state = 'active') AND transactions.type = 'video_purchase' " +
      conditionalWhere;
    await globalModel.custom(req, sqlQuery, condition).then(result => {
      results = result;
    });
  ...
  const paggingData = pagging.create(req, totalCount, page, '', LimitNum);
  const url = req.originalUrl.replace(process.env.ADMIN_SLUG, '');
  res.render('admin/videos/sold-videos', {
...
exports.deleteSoldVideo = async (req, res) => {
  ...
  await globalModel
    .custom(req, 'DELETE FROM transactions WHERE transaction_id = ?', [id])
    .then(result => {
      res.redirect(backURL);
      return;
    });
...
exports.artists = async (req, res) => {
  ...
  const url = req.originalUrl.replace(process.env.ADMIN_SLUG, '');
  res.render('admin/videos/artists', {
    imageSuffix: imageSuffix,
    results: artists,
    title: 'Manage Videos Artists',
    nav: url,
  });
};

exports.swipeVideo = async (req, res) => {
  //get all artists
  let artists = [];
  ...
  const url = req.originalUrl.replace(process.env.ADMIN_SLUG, '');
  res.render('admin/videos/swipe', {
    imageSuffix: imageSuffix,
    results: artists,
    title: 'Manage Videos Swipe',
    nav: url,
  });
};
...
exports.changeOrder = async (req, res) => {
  ...
  if (id || nextid) {
    let categoryData = {};
    await globalModel
      .custom(req, 'SELECT * from categories WHERE category_id = ?', [id])
      .then(result => {
    ...
    if (categoryTypeId) {
      await globalModel
        .custom(
          req,
          'SELECT category_id FROM categories WHERE show_video = 1 AND ' +
            categoryType +
            ' = ?',
          [categoryTypeId],
        )
      ...
      await globalModel
        .custom(
          req,
          'SELECT category_id FROM categories WHERE show_video = 1 AND  subcategory_id = 0 AND subsubcategory_id = 0',
        )
        .then(results => {
          Object.keys(results).forEach(function (key) {
            let result = JSON.parse(JSON.stringify(results[key]));
...
exports.deleteCategories = async (req, res) => {
  ...
  let categoryData = {};
  await globalModel
    .custom(req, 'SELECT * from categories WHERE category_id = ?', [
      category_id,
    ])
    .then(result => {
      categoryData = JSON.parse(JSON.stringify(result));
      ...
      if (categoryData.subsubcategory_id == 0) {
        if (categoryData.subcategory_id != 0) {
          ...
          globalModel
            .custom(
              req,
              'SELECT * from categories WHERE subsubcategory_id = ?',
            ...
            .then(result => {
              result.forEach(cat => {
                ...
                globalModel.custom(
                  req,
                  'UPDATE videos SET subsubcategory_id = 0 WHERE subsubcategory_id = ?',
                  [cat.category_id],
                );
              });
              globalModel.custom(
                req,
                'DELETE from categories WHERE subsubcategory_id = ?',
                [categoryData.category_id],
              );
            });
        } else {
          ...
          globalModel
            .custom(req, 'SELECT * from categories WHERE subcategory_id = ?', [
              categoryData.category_id,
            ])
            .then(result => {
              result.forEach(cat => {
                ...
                globalModel.custom(
                  req,
                  'UPDATE videos SET subcategory_id = 0 WHERE subcategory_id = ?',
                  [cat.category_id],
                );
              });
              globalModel.custom(
                req,
                'DELETE from categories WHERE subcategory_id = ?',
                [categoryData.category_id],
              );
            });
        }
  ...
  await globalModel
    .delete(req, 'categories', 'category_id', category_id)
    .then(result => {
...
exports.categories = async (req, res) => {
  ...
  res.render('admin/videos/categories', {
    imageSuffix: imageSuffix,
    nav: url,
    title: 'Manage Videos Categories',
    categories: categories,
  });
};
...
exports.addCategories = async (req, res) => {
  ...
  if (Object.keys(req.body).length === 0 && category_id) {
    res.render('admin/videos/editCategories', {
      imageSuffix: imageSuffix,
  ...
  if (!category_id) {
    if (cat_id != -1) {
      ...
      if (catData.subcategory_id == 0) {
        ...
        parentId = cat_id;
        await categoryModel
          .orderNext(req, res, { subcat_id: cat_id })
          .then(result => {
        ...
        tableSeprator = '--&nbsp;';
        await categoryModel
          .orderNext(req, res, { subsubcat_id: cat_id })
          .then(result => {
            categoryData['order'] = result ? result : 1;
          });
        parentId = cat_id;
      ...
      seprator = '';
      await categoryModel
        .orderNext(req, res, { category_id: true })
        .then(result => {
          categoryData['order'] = result ? result : 1;
        });
      tableSeprator = '';
  ...
  if (!category_id) {
    ...
    if (req.fileName) {
      data =
        '<img  style="height: 50px;width:50px;margin-bottom: 10px;"  src="' +
        categoryData['image'] +
        '" />';
    } else {
      data = '---';
    ...
    let editCat =
      ...
      process.env.ADMIN_SLUG +
      '/videos/categories/add/' +
      categoryId +
      '">Edit</a>';
    let deleteCat =
      ' <a class="btn btn-danger btn-xs" onclick="preDeleteFn(this)" data-id="' +
      categoryId +
    ...
    let tableData =
      ...
      '<div class="hidden" style="display:none" id="inline_' +
      categoryId +
      '"><div class="parent">' +
      parentId +
      '</div></div></td><td>' +
    ...
    await globalModel
      .update(req, categoryData, 'categories', 'category_id', category_id)
      .then(category => {
...
exports.levels = async (req, res) => {
  ...
  if (flag != 'public') {
    let formFieldsPublic = {
      create: fields.string({
        choices: {
          1: 'Yes, allow to upload videos',
          0: 'No, do not allow to upload videos',
        },
        widget: widgets.select({ classes: ['select'] }),
        label: 'Allow member to upload videos',
        fieldsetClasses: 'form_fieldset',
        cssClasses: { field: ['form-group'] },
        value: cacheContent['video.create']
          ? cacheContent['video.create'].toString()
          : 1,
      }),
    };
  ...
  let formFieldsView = {
    view: fields.string({
      ...
      fieldsetClasses: 'form_fieldset',
      cssClasses: { field: ['form-group'] },
      value: cacheContent['video.view']
        ? cacheContent['video.view'].toString()
        : 1,
    }),
  };
  ...
  if (flag != 'public') {
    let formFields1 = {
      edit: fields.string({
        ...
        fieldsetClasses: 'form_fieldset',
        cssClasses: { field: ['form-group'] },
        value: cacheContent['video.edit']
          ? cacheContent['video.edit'].toString()
          : 1,
      }),
      delete: fields.string({
        ...
        fieldsetClasses: 'form_fieldset',
        cssClasses: { field: ['form-group'] },
        value: cacheContent['video.delete']
          ? cacheContent['video.delete'].toString()
          : 1,
      }),
      quota: fields.string({
        label:
          'No. Of videos member can upload to selected level? Enter 0 for unlimited',
        validators: [validators.integer('Enter integer value only.')],
        cssClasses: { field: ['form-group'] },
        widget: widgets.text({ classes: ['form-control'] }),
        value: cacheContent['video.quota']
          ? cacheContent['video.quota'].toString()
          : 0,
      }),
      storage: fields.string({
        ...
        widget: widgets.select({ classes: ['select'] }),
        cssClasses: { field: ['form-group'] },
        value: cacheContent['video.storage']
          ? cacheContent['video.storage'].toString()
          : 0,
      }),
      ...
      embedcode: fields.string({
        ...
        fieldsetClasses: 'form_fieldset',
        cssClasses: { field: ['form-group'] },
        value: cacheContent['video.embedcode']
          ? cacheContent['video.embedcode'].toString()
          : 0,
      }),
      sponsored: fields.string({
        ...
        fieldsetClasses: 'form_fieldset',
        cssClasses: { field: ['form-group'] },
        value: cacheContent['video.sponsored']
          ? cacheContent['video.sponsored'].toString()
          : 1,
      }),
      featured: fields.string({
        ...
        fieldsetClasses: 'form_fieldset',
        cssClasses: { field: ['form-group'] },
        value: cacheContent['video.featured']
          ? cacheContent['video.featured'].toString()
          : 1,
      }),
      hot: fields.string({
        choices: { 1: 'Yes', 0: 'No' },
        ...
        fieldsetClasses: 'form_fieldset',
        cssClasses: { field: ['form-group'] },
        value: cacheContent['video.hot']
          ? cacheContent['video.hot'].toString()
          : 1,
      }),
      ...
      auto_approve: fields.string({
        ...
        fieldsetClasses: 'form_fieldset',
        cssClasses: { field: ['form-group'] },
        value: cacheContent['video.auto_approve']
          ? cacheContent['video.auto_approve'].toString()
          : 1,
      }),
      donation: fields.string({
        ...
        fieldsetClasses: 'form_fieldset',
        cssClasses: { field: ['form-group'] },
        value: cacheContent['video.donation']
          ? cacheContent['video.donation'].toString()
          : 1,
      }),
      sell_videos: fields.string({
        ...
        fieldsetClasses: 'form_fieldset',
        cssClasses: { field: ['form-group'] },
        value: cacheContent['video.sell_videos']
          ? cacheContent['video.sell_videos'].toString()
          : 1,
      }),
    };
  ...
  reg_form.handle(req, {
    success: function (form) {
      permission
        .insertUpdate(req, res, form.data, level_id, 'video')
        .then(result => {
          res.send({
            success: 1,
            message: 'Operation performed successfully.',
            url: process.env.ADMIN_SLUG + '/videos/levels/' + level_id,
          });
        });
    },
    error: function (form) {
    ...
    other: function (form) {
      res.render('admin/videos/levels', {
        nav: url,
        reg_form: reg_form,
        title: 'Videos Member Role Settings',
      });
    },
  });
};
...
exports.settings = async (req, res) => {
  ...
  await fileManager
    .findAll(req, { column: 'path', like: 'video' })
    .then(result => {
      result.forEach(res => {
        let url = res.path
          .split(/(\\|\/)/g)
  ...
  var reg_form = forms.create(
    {
      ...
      video_upload_label: fields.string({
        widget: widgets.label({
          content:
            'Do you want to allow user on your website to upload videos from their computer.',
        }),
        cssClasses: { field: ['form-group', 'form-description'] },
      }),
      video_ffmpeg_path: fields.string({
        ...
        value: req.loguserallowed
          ? '****'
          : settings.getSetting(req, 'video_ffmpeg_path', ''),
      }),
      video_ffmpeg_path_label: fields.string({
        widget: widgets.label({
          content:
            'This will compress, convert, and optimise videos to mp4. Please contact your hosting provider if you can not install FFMPEG.',
        }),
        cssClasses: { field: ['form-group', 'form-description'] },
      }),
      ...
      video_upload_videos_type: fields.string({
        ...
        value: settings
          .getSetting(req, 'video_upload_videos_type', '')
          .split(','),
      }),
      video_upload_videos_type_label: fields.string({
        widget: widgets.label({
          content:
            'Choose from above setting in which format you want to upload video on your website. If you enable higher resolution videos than your server must be that much capable to convert those resolution videos.',
        }),
        cssClasses: { field: ['form-group', 'form-description'] },
      }),
      ...
      video_conversion_type_label: fields.string({
        widget: widgets.label({
          content:
            'Using a slower preset gives you better compression, or quality per filesize, whereas faster presets give you worse compression and higher filesize.',
        }),
        cssClasses: { field: ['form-group', 'form-description'] },
      }),
      ...
      enable_iframely_label: fields.string({
        widget: formFunctions.makeClickable({
          content:
            'Iframely integration works with the ‘Import Videos’ feature and enhances import functionality. [0] to learn about all the awesome features Iframely provides.',
          replace: [
            {
              0: '<a href="https://iframely.com/features" target="_blank">Click here</a>',
            },
          ],
        }),
        cssClasses: { field: ['form-group', 'form-description'] },
      ...
      iframely_api_key: fields.string({
        ...
        value: req.loguserallowed
          ? '****'
          : settings.getSetting(req, 'iframely_api_key', ''),
      }),
      ...
      iframely_disallow_sources_label: fields.string({
        widget: formFunctions.makeClickable({
          content:
            'You can use this setting to control which sites should not be allowed in this section. Enter the domains of the sites (separated by commas) that you do not want to allow for Video Source. ',
        }),
        cssClasses: { field: ['form-group', 'form-description'] },
      }),
      ...
      enable_youtube_import: fields.string({
        ...
        cssClasses: { field: ['form-group'] },
        value: settings
          .getSetting(req, 'enable_youtube_import', '0')
          .toString(),
      }),
      enable_youtube_import_label: fields.string({
        widget: widgets.label({
          content: 'Do you want to enable youtube import on your website',
        }),
        cssClasses: { field: ['form-group', 'form-description'] },
      }),
      youtube_api_key: fields.string({
        ...
        value: req.loguserallowed
          ? '****'
          : settings.getSetting(req, 'youtube_api_key', ''),
      }),
      youtubelabel: fields.string({
        widget: formFunctions.makeClickable({
          content: '[0] to learn how to create Youtube API key.',
          replace: [
            {
              0:
                '<a href="' +
                process.env.PUBLIC_URL +
                '/Documentation/youtube" target="_blank">Click here</a>',
            },
          ],
        }),
        cssClasses: { field: ['form-group', 'form-description'] },
      ...
      twitchlabel: fields.string({
        widget: formFunctions.makeClickable({
          content: '[0] to learn how to create Twitch API key.',
          replace: [
            {
              0:
                '<a href="' +
                process.env.PUBLIC_URL +
                '/Documentation/twitch" target="_blank">Click here</a>',
            },
          ],
        }),
        cssClasses: { field: ['form-group', 'form-description'] },
      ...
      enable_twitch_import_label: fields.string({
        widget: widgets.label({
          content: 'Do you want to enable twitch import on your website',
        }),
        cssClasses: { field: ['form-group', 'form-description'] },
      }),
      twitch_api_key: fields.string({
        ...
        value: req.loguserallowed
          ? '****'
          : settings.getSetting(req, 'twitch_api_key', ''),
      }),
      ...
      enable_facebook_import: fields.string({
        ...
        cssClasses: { field: ['form-group'] },
        value: settings
          .getSetting(req, 'enable_facebook_import', '0')
          .toString(),
      }),

      facebooklabel: fields.string({
        widget: formFunctions.makeClickable({
          content: '[0] to learn how to create Facebook API key.',
          replace: [
            {
              0:
                '<a href="' +
                process.env.PUBLIC_URL +
                '/Documentation/facebook" target="_blank">Click here</a>',
            },
          ],
        }),
        cssClasses: { field: ['form-group', 'form-description'] },
      ...
      enable_facebook_import_label: fields.string({
        widget: widgets.label({
          content: 'Do you want to enable facebook import on your website',
        }),
        cssClasses: { field: ['form-group', 'form-description'] },
      }),
      facebook_client_id: fields.string({
        ...
        value: req.loguserallowed
          ? '****'
          : settings.getSetting(req, 'facebook_client_id', ''),
      }),
      ...
      facebook_client_secret: fields.string({
        ...
        value: req.loguserallowed
          ? '****'
          : settings.getSetting(req, 'facebook_client_secret', ''),
      }),
      ...
      videotip_commission_type: fields.string({
        ...
        cssClasses: { field: ['form-group'] },
        value: settings
          .getSetting(req, 'videotip_commission_type', '1')
          .toString(),
      }),
      videotip_commission_value: fields.string({
        label:
          'Get Commission from tip videos (put 0 if you not want comission.)',
        cssClasses: { field: ['form-group'] },
        widget: widgets.text({ classes: ['form-control'] }),
      ...
      video_commission_type: fields.string({
        ...
        cssClasses: { field: ['form-group'] },
        value: settings
          .getSetting(req, 'video_commission_type', '1')
          .toString(),
      }),
      video_commission_value: fields.string({
        label:
          'Get Commission from sell videos (put 0 if you not want comission.)',
        cssClasses: { field: ['form-group'] },
        widget: widgets.text({ classes: ['form-control'] }),
      ...
      video_upload_limit: fields.string({
        label:
          'Maximum Upload Limit of Videos. Enter value in MB (Enter 0 for unlimited.)',
        validators: [validators.integer('Enter integer value only.')],
        cssClasses: { field: ['form-group'] },
        widget: widgets.text({ classes: ['form-control'] }),
      ...
      video_autoplay: fields.string({
        ...
        widget: widgets.select({ classes: ['select'] }),
        label:
          'Enable Autoplay feature on video view page (work only when above Iframely setting is set "NO")?',
        fieldsetClasses: 'form_fieldset',
        cssClasses: { field: ['form-group'] },
      ...
      video_miniplayer: fields.string({
        ...
        widget: widgets.select({ classes: ['select'] }),
        label:
          'Enable Miniplayer feature on videos (work only when above Iframely setting is set "NO")?',
        fieldsetClasses: 'form_fieldset',
        cssClasses: { field: ['form-group'] },
      ...
      video_donation_label: fields.string({
        widget: widgets.label({
          content:
            'Enabling this feature user can request for donation on uploaded videos',
        }),
        cssClasses: { field: ['form-group', 'form-description'] },
      }),
      ...
      video_comment_dislike: fields.string({
        ...
        cssClasses: { field: ['form-group'] },
        value: settings
          .getSetting(req, 'video_comment_dislike', '1')
          .toString(),
      }),
      ...
      video_category_default_photo: fields.string({
        ...
        cssClasses: { field: ['form-group'] },
        value: settings
          .getSetting(req, 'video_category_default_photo', '')
          .toString(),
      }),
      ...
      artist_cnt_label: fields.string({
        widget: formFunctions.makeClickable({
          content:
            '<h2 style="text-align: center;margin: 40px;text-decoration: underline;">Video Artists Settings</h2>',
          replace: [],
        }),
        cssClasses: { field: ['form-group', 'form-description'] },
      }),
      ...
      video_artists_label: fields.string({
        widget: widgets.label({
          content:
            'If you enable this feature then user are able to select artists in the videos.',
        }),
        cssClasses: { field: ['form-group', 'form-description'] },
      }),
      ...
      video_artist_favourite: fields.string({
        ...
        cssClasses: { field: ['form-group'] },
        value: settings
          .getSetting(req, 'video_artist_favourite', '1')
          .toString(),
      }),
      ...
      video_artist_comment_like: fields.string({
        ...
        cssClasses: { field: ['form-group'] },
        value: settings
          .getSetting(req, 'video_artist_comment_like', '1')
          .toString(),
      }),
      ...
      video_artist_comment_dislike: fields.string({
        ...
        cssClasses: { field: ['form-group'] },
        value: settings
          .getSetting(req, 'video_artist_comment_dislike', '1')
          .toString(),
      }),
    },
    { validatePastFirstError: true },
  ...
  reg_form.handle(req, {
    success: function (form) {
      if (form.data['video_commission_type'] == '1') {
        if (parseFloat(form.data['video_commission_value']) < 0) {
          res.send({
            errors: { video_commission_value: 'Please enter valid value.' },
          });
          return;
        }
      } else if (form.data['video_commission_type'] == '2') {
        if (
          parseFloat(form.data['video_commission_value']) > 99.99 ||
          parseFloat(form.data['video_commission_value']) < 0
        ) {
          //error
          res.send({
            errors: { video_commission_value: 'Please enter valid value.' },
      ...
      form.data['video_commission_value'] = parseInt(
        form.data['video_commission_value'],
      );
    ...
    other: function (form) {
      res.render('admin/videos/settings', {
        nav: url,
        reg_form: reg_form,
      ...
      function (error, itemResults) {
        ...
        youTube.getById(ids.join(','), function (error, results) {
          ...
          if (itemResults.prevPageToken) {
            ...
            valid = true;
            paggingData +=
              '<li class="page-item previouspage" id="previous-page"><a class="page-link" href="' +
              previousUrl +
              '">Previous</a></li>';
          }
          if (itemResults.nextPageToken) {
            ...
            valid = true;
            paggingData +=
              '<li class="page-item nextpage"><a class="page-link" id="next-page" href="' +
              nextUrl +
              '">Next</a></li>';
          }
          paggingData += '</ul>';
          ...
          res.render('admin/videos/youtube', {
            type: 1,
            adminLink: process.env.ADMIN_SLUG,
            results: videos,
            categories: categories,
            query: query,
            paggingData: paggingData,
...
exports.createImportedVideos = async (req, res, next) => {
  ...
  if (user_email) {
    await globalModel
      .custom(req, 'SELECT * FROM users where email = ?', [user_email])
      .then(result => {
  ...
  videoData.forEach(video => {
    globalModel
      .custom(req, 'SELECT * FROM videos WHERE type = 1 AND code = ?', [
        video.video_id,
      ])
      .then(resultData => {
        if (resultData) {
...
exports.dailyMotionImport = async (req, res, next) => {
  ...
  if (query.search) {
    ...
    axios
      .get(
        'https://api.dailymotion.com/videos?fields=id,tags,duration,description,thumbnail_url,title&page=' +
          page +
          '&limit=' +
          LimitNum +
          '&search=' +
      ...
      .then(function (itemResults) {
        ...
        res.render('admin/videos/youtube', {
          ...
          paggingData: paggingData,
          nav: url,
          title: 'Import Dailymotion Videos',
        });
      });
  } else {
    const url = req.originalUrl.replace(process.env.ADMIN_SLUG, '');
