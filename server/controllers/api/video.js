...
(axios = require('axios')), (settingModel = require('../../models/settings'));
      ...
      .then(async result => {
        if (result) {
          ...
          if (results && results.length) {
            ...
            if (!videoObj.image) {
              insertObject['image'] =
                '/LiveApp/previews/' + req.body.vodName + '.png';
              if (n > -1) {
      ...
      var dataStart = {
        ...
        clientRequest: {
          ...
          storageConfig: {
            ...
            fileNamePrefix: ['upload', 'livestreamings'],
          },
        },
      };
...
exports.artists = async (req, res) => {
  ...
  if (videoId) {
    //uploaded
    await globalModel
      .custom(req, 'SELECT * FROM videos WHERE video_id = ?', videoId)
      .then(async result => {
        if (result && result.length) {
...
exports.create = async (req, res) => {
  ...
  let subCatId = req.body.subcategory_id * 1;
  ...
  let catObj;
  if (subCatId) {
    catObj = await categoryModel?.findById(subCatId, req, res);
  } else {
    catObj = await categoryModel?.findById(catId, req, res);
  }
  ...
  if (req.body.parent_video) {
    insertObject['parent_video'] = req.body.parent_video;
  }
  ...
  if (Object.keys(videoObject).length && videoId) {
    if (!req.fileName && !req.body.fromEdit) {
      ...
      if (image) {
        ...
        await resizeObj
          .save('/upload/images/videos/video/' + newFileName)
          .then(async res => {
            ...
            if (
              ...
              await s3Upload(
                req,
                req.serverDirectoryPath + insertObject['image'],
                insertObject['image'],
              )
                .then(result => {
                  //remove local file
  ...
  //changed the custom url here
  if (!Object.keys(videoObject).length || !videoObject.custom_url) {
    insertObject['custom_url'] = catName + '-' + vidId;
    insertObject['is_sponsored'] =
  ...
  if (videoId) {
    //update existing video
    await globalModel
      .update(req, insertObject, 'videos', 'video_id', videoId)
      .then(async result => {
        ...
        if (insertObject['category_id'] != videoObject.category_id) {
          globalModel
            .custom(
              req,
              'UPDATE categories SET item_count = item_count - 1 WHERE category_id = ' +
                videoObject['category_id'],
            )
            .then(result => {})
            .catch(err => {});
        }
        if (
          insertObject['subcategory_id'] != videoObject.subcategory_id ||
          (insertObject['category_id'] == 0 && videoObject.subcategory_id)
        ) {
          globalModel
            .custom(
              req,
              'UPDATE categories SET item_count = item_count - 1 WHERE category_id = ' +
                videoObject['subcategory_id'],
            )
            .then(result => {})
            .catch(err => {});
        }
        if (
          insertObject['subsubcategory_id'] != videoObject.subsubcategory_id ||
          (insertObject['subcategory_id'] == 0 && videoObject.subsubcategory_id)
        ) {
          globalModel
            .custom(
              req,
              'UPDATE categories SET item_count = item_count - 1 WHERE category_id = ' +
                videoObject['subsubcategory_id'],
            )
            .then(result => {})
            .catch(err => {});
        }
        if (
          insertObject['category_id'] != videoObject.category_id &&
          insertObject['category_id'] > 0
        ) {
          globalModel
            .custom(
              req,
              'UPDATE categories SET item_count = item_count + 1 WHERE category_id = ' +
                insertObject['category_id'],
            )
            .then(result => {})
            .catch(err => {});
          if (
            insertObject['subcategory_id'] != videoObject.subcategory_id &&
            insertObject['subcategory_id'] > 0
          ) {
            globalModel
              .custom(
                req,
            ...
            if (
              insertObject['subsubcategory_id'] !=
                videoObject.subsubcategory_id &&
              insertObject['subsubcategory_id'] > 0
            ) {
              globalModel
                .custom(
                  req,
      ...
      async function (tipData, i, callback) {
        ...
        if (price > 0) {
          ...
          if (!tip.tip_id || forceInsert) {
            ...
            //update
            await globalModel
              .update(req, insertObject, 'tips', 'tip_id', videoId)
              .then(result => {});
          }
        } else if (tip.tip_id) {
          //delete entry
  ...
  return new Promise(async (resolve, reject) => {
    ...
    await module.exports
      ...
      .then(async result => {
        ...
        await globalModel
          .update(req, updatedObject, 'videos', 'video_id', videoId)
          .then(result => {})
          .catch(error => {});
        is_validVideo = true;
      })
...
exports.executeFFMPEG = async (
  ...
  console.log('video execute ffmpeg');
  console.log(
    ' params ',
    command,
    filePath,
