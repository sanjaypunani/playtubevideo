...
exports.purchase = async (req, res) => {
  ...
  await videoModel
    .checkVideoPurchased(
      { id: video.video_id, owner_id: req.user.user_id },
      req,
    )
    .then(result => {
      if (result) {
  ...
  data['returnUrl'] =
    `${process.env.PUBLIC_URL}/videos/successulPayment/` + video.video_id;
  data['cancelUrl'] =
    `${process.env.PUBLIC_URL}/videos/cancelPayment/` + video.video_id;
  ...
  await globalModel
    .custom(
      req,
      "DELETE FROM orders WHERE owner_id = ? AND state = 'initial' AND source_type = 'video_purchase'",
      [req.user.user_id],
    )
    .then(result => {});
  //create order
...
exports.successul = async (req, res, next) => {
  ...
  if (
    !req.user ||
    !req.session.videotokenUserPayment ||
    !req.session.video_user_id ||
    !req.session.videoId ||
    !req.session.orderId
  ) {
    ...
    await oneTimePaypal
      .execute(req, res, PayerID, { price: parseFloat(video.price).toFixed(2) })
      .then(async executeResult => {
        if (executeResult) {
          let commission_amount = 0;
          let commissionType = parseFloat(
            req.appSettings['video_commission_type'],
          );
          let commissionTypeValue = parseFloat(
            req.appSettings['video_commission_value'],
          );
          ...
          await globalModel
            .create(
              req,
              {
                type: 'video_purchase',
                id: video.video_id,
                owner_id: req.session.video_user_id,
                package_id: 0,
                status: executeResult.state.toLowerCase(),
                creation_date: currentDate,
                modified_date: currentDate,
                gateway_profile_id: executeResult.transaction_id,
            ...
            .then(async result => {
              ...
              await videoPurchaseModel
                .insertTransaction(req, {
                  ...
                  gateway_transaction_id: executeResult.transaction_id,
                  owner_id: req.session.video_user_id,
                  state: executeResult.state.toLowerCase(),
...
exports.categories = async (req, res) => {
  ...
  await categoryModel
    .findAll(req, {
      type: req.query.type,
      orderBy: ' categories.item_count DESC ',
    })
    .then(result => {
  ...
  await commonFunction.updateMetaData(req, {
...
exports.category = async (req, res) => {
  ...
  await categoryModel
    .findByCustomUrl(
      { id: req.query.categoryId, type: req.query.type },
      req,
      res,
    )
    .then(result => {
  ...
  if (category.subcategory_id == 0 && category.subsubcategory_id == 0) {
    ...
    await categoryModel
      .findAll(req, {
        type: 'video',
        subsubcategory_id: category.category_id,
        item_count: 1,
      })
      .then(result => {
        if (result) {
...
exports.browse = async (req, res) => {
  ...
  if (categories.length > 0) req.query.categories = categories;
  ...
  } else req.app.render(req, res, '/videos', req.query);
};
exports.view = async (req, res) => {
  ...
  if (playlistId) {
    await playlistModel
      .findByCustomUrl(playlistId, req)
      .then(async result => {
        if (result) {
          playlist = result;
          await userModel
            .findById(result.owner_id, req, res)
            .then(result => {
  ...
  if (
    (playlistId && !Object.keys(playlist).length) ||
    (playlistId &&
      Object.keys(playlist).length &&
      playlist.private == 1 &&
      (!req.user || req.user.user_id != playlist.owner_id))
  ) {
    if (req.query.data) {
      res.send({ data: req.query, pagenotfound: 1 });
      return;
  ...
  if (playlistId) {
    //get playlist videos
    await videoModel
      .getVideos(req, { playlist_id: playlist.playlist_id })
      .then(result => {
  ...
  if (referer) {
    ...
    if (a.hostname != currentHost.hostname) {
      ...
      insertObject['ip'] =
        typeof req.headers != 'undefined' && req.headers['x-forwarded-for']
          ? req.headers['x-forwarded-for']
          : req.connection && req.connection.remoteAddress
          ? req.connection.remoteAddress
          : '';
      insertObject['creation_date'] = new Date(
  ...
  if (
    !Object.keys(video).length ||
    ((video.status != 1 || video.approve != 1) &&
      (!req.user ||
        (video.owner_id != req.user.user_id &&
          req.levelPermissions['video.view'] != 2)))
  ) {
    if (req.query.data) {
      res.send({ data: req.query, pagenotfound: 1 });
      return;
  ...
  await privacyLevelModel
    .findBykey(req, 'video', 'sell_videos', video.level_id)
    .then(result => {
      if (result == 1) video.sell_videos = true;
    });
  await privacyLevelModel
    .findBykey(req, 'video', 'donation', video.level_id)
    .then(result => {
      if (result == 1) video.donation = result;
    });

  await privacyModel
    .permission(req, 'video', 'delete', video)
    .then(result => {
      video.canDelete = result;
    })
    .catch(err => {});
  await privacyModel
    .permission(req, 'video', 'edit', video)
    .then(result => {
  ...
  if (
    parseInt(settings['live_stream_start']) == 1 &&
    parseInt(video.is_livestreaming) == 1
  ) {
    ...
    let videoTime = new Date(
      dateTime.create(video.creation_date).format('Y-m-d H:M:S'),
    ).getTime();
  ...
  if (video.approve == 1) {
    if (
      ...
      let sql =
        'SELECT advertisements_user.* from advertisements_user LEFT JOIN users ON users.user_id = advertisements_user.owner_id  LEFT JOIN userdetails ON users.user_id = userdetails.user_id WHERE users.active = 1 AND users.approve = 1 AND advertisements_user.approve = 1 AND advertisements_user.completed = 1 ';
      sql +=
        ' AND ( CASE WHEN advertisements_user.type = 1 AND users.wallet >= ' +
        ads_cost_perclick +
        ' THEN true ';
      sql +=
        ' WHEN advertisements_user.type = 2 AND users.wallet >= ' +
        ads_cost_perview +
        ' THEN true ';
      sql +=
        ' ELSE false  END ) AND ( category_id = 0 OR category_id = ? ) AND ( subcategory_id = 0 OR subcategory_id = ? ) AND ( subsubcategory_id = 0 OR subsubcategory_id = ? )';
      if (video.adult == 1) {
        sql +=
          ' AND ( advertisements_user.adult IS NULL OR  advertisements_user.adult = 1) ';
      } else if (video.adult == 0) {
        sql +=
          ' AND ( advertisements_user.adult IS NULL OR  advertisements_user.adult = 0) ';
      }
        ...
        .then(result => {
          if (result) {
            ...
            if (adsResults && adsResults.length) {
              ...
              if (ads.type == 2) {
                adsTransaction.adSpent(req, {
                  owner_id: ads.owner_id,
                  ad_id: ads.ad_id,
                  amount: req.query.appSettings['ads_cost_perview'],
                  type: 'view',
      ...
      if (!req.query.userAdVideo) {
        //fetch admin ads
        let sql =
          'SELECT * from advertisements_admin WHERE advertisements_admin.active = 1 ';

        sql +=
          ' AND ( category_id = 0 OR category_id = ? ) AND ( subcategory_id = 0 OR subcategory_id = ? ) AND ( subsubcategory_id = 0 OR subsubcategory_id = ? ) ';
            ...
            if (result) {
              ...
              if (adsResults && adsResults.length) {
                ...
                globalModel
                  .custom(
                    req,
                    'UPDATE advertisements_admin SET view_count = view_count + 1 WHERE ad_id = ?',
                    [adsResults[0].ad_id],
                  )
                  .then(result => {});
              }
  ...
  if (
    ...
    await artistModel
      .findByIds(video.artists, req, res)
      .then(result => {
        if (result) video.artists = result;
      })
      .catch(error => {});
  }
  //video user details
  await userModel
    .findById(video.owner_id, req, res)
    .then(result => {
      video.owner = result;
    })
    .catch(error => {});
  ...
  if (video.category_id) {
    await categoryModel
      .findById(video.category_id, req, res)
      .then(result => {
        if (result) {
    ...
    if (video.subcategory_id) {
      await categoryModel
        .findById(video.subcategory_id, req, res)
        .then(result => {
          if (result) {
      ...
      if (video.subsubcategory_id) {
        await categoryModel
          .findById(video.subsubcategory_id, req, res)
          .then(result => {
            if (result) {
  ...
  if (parseInt(req.appSettings['video_tip']) == 1 && video.approve == 1) {
    ...
    await videoModel
      ...
      .then(result => {
        if (result && result.length > 0) video.tips = result;
      });
    ...
    await videoModel
      .donors(
        { video_id: video.video_id, limit: LimitNum, offset: offsetDonor },
        req,
      )
      .then(result => {
        let pagging = false;
    ...
    if (!video.parent_video) {
      req.query.mainVideo = video;
    } else {
      await videoModel
    ...
    // All Languages videos
    if (req.query.mainVideo) {
      await videoModel
        .getVideos(req, {
          parent_video: req.query.mainVideo.video_id,
          subLanguageAlso: true,
        })
        .then(result => {
          req.query.allLanguagesVideo = result;
        })
        .catch(error => {
          showVideo = false;
        });
      req.query.allLanguagesVideo = [
        req.query.mainVideo,
        ...req.query.allLanguagesVideo,
      ];

      req.query.currentLanguageVideo = req.query.allLanguagesVideo?.find(
        item => item?.language === req.query.lng,
      );
    }
  }
  ...
  if (
    ...
    (req.appSettings['video_adult'] != 1 ||
      video.adult == 0 ||
      (video.adult == 1 && req.query.adultAllowed))
  ) {
    req.query.video = video;

    if (req.query.currentLanguageVideo) {
      req.query.video.custom_url = req.query.currentLanguageVideo.custom_url;
    }
  ...
  if (!req.query.relatedVideos || req.query.relatedVideos.length < 10) {
    await videoModel
      .getVideos(req, {
        orderby: ' view_count desc ',
        not_videos_id: req.query.relatedVideos,
        limit:
          10 -
          (req.query.relatedVideos && req.query.relatedVideos.length
            ? req.query.relatedVideos.length
            : 0),
        not_video_id: video.video_id,
      })
      .then(result => {
...
exports.adClicked = async (req, res, next) => {
  ...
  if (type == 'admin') {
    await globalModel
      .custom(req, 'SELECT * FROM advertisements_admin WHERE ad_id = ?', [id])
      .then(result => {
        if (result) {
          const adsResults = JSON.parse(JSON.stringify(result));
          if (adsResults && adsResults.length) {
            ...
            globalModel
              .custom(
                req,
                'UPDATE advertisements_admin SET click_count = click_count + 1 WHERE ad_id = ?',
                [ad.ad_id],
              )
              .then(result => {});
            valid = true;
    ...
    await globalModel
      .custom(
        req,
        'SELECT advertisements_user.*,users.level_id FROM advertisements_user LEFT JOIN users ON users.user_id = advertisements_user.owner_id WHERE users.user_id IS NOT NULL AND users.active = 1 AND users.approve = 1 AND  ad_id = ?',
        [id],
      )
      .then(async result => {
        if (result) {
          ...
          if (adsResults && adsResults.length) {
            ...
            if (ads.type == 1) {
              ...
              adsTransaction.reduceOwnerWalletAmount(req, {
                owner_id: ads.owner_id,
                amount: req.appSettings['ads_cost_perview'],
              });
            }
            adsTransaction.adSpent(req, {
              owner_id: ads.owner_id,
...
exports.create = async (req, res) => {
  ...
  if (videoId) {
    ...
    await videoModel
      .findByCustomUrl(videoId, req, res, true)
      .then(async video => {
  ...
  if (categories.length > 0) req.query.videoCategories = categories;
  ...
  if (parseInt(req.appSettings['video_tip']) == 1 && req.query.editItem) {
    ...
    await videoModel
      .getTips(req, {
        resource_id: req.query.editItem.video_id,
        resource_type: 'video',
        view: true,
      })
      .then(result => {
        if (result && result.length > 0) req.query.editItem.tips = result;
      });
  }
  ...
  if (req.appSettings['video_sell']) {
    await privacyLevelModel
      .findBykey(req, 'video', 'sell_videos', req.user.level_id)
      .then(result => {
        if (result == 1) req.query.sell_videos = result;
      });
  }
  ...
  if (settingModel.getSetting(req, 'video_artists', 1) != 0) {
    await artistModel.findAll(req, { type: 'video' }).then(artist => {
      if (artist) req.query.videoArtists = artist;
    });
  }
  ...
  return new Promise(async (resolve, reject) => {
    ...
    var config = {
      ...
      url:
        ...
        ':5080/LiveApp/rest/v2/broadcasts/create',
      headers: {
        ...
        if (response.data.status == 'created') {
          ...
          var config = {
            ...
            url:
              ...
              ':5080/LiveApp/rest/v2/broadcasts/' +
              streamingId +
              '/token?expireDate=' +
