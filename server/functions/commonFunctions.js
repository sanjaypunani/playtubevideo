...
exports.updateMetaData = async (req, data) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (
      ...
      req.appSettings['page_default_image']
    ) {
    ...
    if (req.query.pageInfo) {
      ...
      if (dataTitle) {
        ...
        if (dataLength > 160 - titleLength) {
          dataTitle =
            exports.truncate(dataTitle, 60 - titleLength - 4) + ' ...';
        }
        req.query.pageInfo.title = dataTitle + title;
      }
      let dataDescription = striptags(data['description']);
      if (dataDescription) {
        let dataDescriptionLength = (dataDescription ? dataDescription : '')
          .length;
      ...
      if (data['keywords']) req.query.pageInfo.keywords = data['keywords'];
      if (data['image']) req.query.pageInfo.image = data['image'];

      if (data['banner']) req.query.pageInfo.banner = data['banner'];
    }
    ...
    if (req.query.pageInfo && req.query.pageInfo.image && ip != '::1') {
      ...
      if (
        image.toString().indexOf('https://') === 0 ||
        image.toString().indexOf('http://') === 0
      ) {
        isOutSide = true;
      }
      if (
        ...
        var options = url.parse(
          isOutSide ? imgUrl : req.query.imageSuffix + req.query.pageInfo.image,
        );
        await exports
          .getImageSize(
            req,
            ...
            isOutSide
              ? imgUrl
              : req.query.imageSuffix + req.query.pageInfo.image,
          )
          .then(dimensions => {
            if (dimensions) {
              req.query.pageInfo.imageHeight = dimensions.height;
...
exports.getImageSize = async (req, options, image) => {
  return new Promise((resolve, reject) => {
    (image.toString().indexOf('https://') === 0 ? https : http).get(
      options,
      function (response) {
        var chunks = [];
        response
          ...
          .on('end', function () {
            try {
              ...
              if (buffer) resolve(sizeOf(buffer));
              else reject(false);
            } catch (err) {
              resolve(false);
            }
...
exports.getGeneralInfo = async (req, res, type, fromNode = false) => {
  ...
  if (agora_app_id && settings['agora_s3_bucket']) {
    req.query.liveStreamingURL =
      'https://' + settings['agora_s3_bucket'] + '.s3.amazonaws.com';
  }
  ...
  if (
    ((type_livestreaming == 1 && agora_app_id) ||
      (type_livestreaming == 0 && settings['antserver_media_url'])) &&
    parseInt(settings['live_stream_start'], 10) == 1
  ) {
  ...
  if (
    ...
    (req.headers['user-agent'].indexOf('MSIE') >= 0 ||
      req.headers['user-agent'].indexOf('Trident') >= 0)
  ) {
    ...
    if (
      ...
      try {
        const dimensions = await sizeOf(
          req.serverDirectoryPath + '/public' + req.query.pageInfo.image,
        );
        req.query.pageInfo.imageHeight = dimensions.height;
        req.query.pageInfo.imageWidth = dimensions.width;
      } catch (e) {
  ...
  req.query.CDN_URL_FOR_STATIC_RESOURCES = process.env
    .CDN_URL_FOR_STATIC_RESOURCES
    ? process.env.CDN_URL_FOR_STATIC_RESOURCES
    : '';
  req.query.audioPassword =
    req.session && req.session.audio ? req.session.audio : [];

  if (req.appSettings['fixed_header'] == 1) {
    //get video categories for sidebar
    let condision = { type: 'video', limit: 5000, onlyCategories: 0 };
    ...
    if (fourceCategory) {
      condision.subcategory_id = fourceCategory.category_id;
      condision.onlyCategories = 0;
    }
    await videoCategoryModel.findAll(req, condision).then(results => {
      if (results && results.length > 0) {
        let newResult = results;
        if (fourceCategory) {
          newResult.push(fourceCategory);
        }
    ...
    if (req.user) {
      ...
      channelModel
        .getFollowedChannels(req, { limit: 5 })
        .then(result => {
          if (result) {
    ...
    await userModel
      .getMembers(req, { limit: 16, orderby: 'random', is_popular: true })
      .then(results => {
        if (results && results.length > 0) {
  ...
  if (!fromNode) {
    ...
    if (req.fromBlog) req.query.tinymceKey = req.appSettings.tinymceKey;
    ...
    if (
      req.appSettings.video_ffmpeg_path &&
      req.appSettings.video_upload == '1'
    ) {
  ...
  req.query.defaultTimezone = req.user
    ? req.user.timezone
  ...
  let enable_theme_design_mode =
    req.levelPermissions &&
    typeof req.levelPermissions['member.enable_theme_design_mode'] !=
      'undefined'
      ? req.levelPermissions['member.enable_theme_design_mode']
      : 1;
  let theme_design_mode =
    req.levelPermissions &&
    typeof req.levelPermissions['member.theme_design_mode'] != 'undefined'
      ? req.levelPermissions['member.theme_design_mode']
      : 3;

  if (theme_design_mode == 3 || theme_design_mode == 4) {
    if (enable_theme_design_mode == 1) req.query.toogleMode = true;
  ...
  if (req.query.h) req.query.searchTitleText = req.query.h;
  ...
  if (req.user) {
    ...
    await packagesModel
      .findAll(req, { column: 'packages.*', enabled: 1, not_default_plan: 1 })
      .then(async result => {
...
exports.deleteUploadedContent = async (req, res, image, type) => {
  ...
  if (
    (upload_system == 's3' || upload_system == 'wisabi') &&
    type != 'locale'
  ) {
    let wisabiStorage = {};
    if (upload_system == 's3') {
      ...
      const wasabiEndpoint = new aws.Endpoint(
        `s3.${req.appSettings.s3_region}.wasabisys.com`,
      );
      wisabiStorage = {
  ...
  fs.unlink(
    req.serverDirectoryPath + '/public/' + trimMainImage,
    function (err) {
      // if (err) {
      //     console.error(err);
      // }
...
exports.deleteImage = async (req, res, image, type, obj) => {
  if (obj) {
    if (obj.video_id) {
      ...
      if (obj.type == 3) {
        ...
        if (obj.video_location) {
          ...
          if (obj['4096p'] == 1) {
            exports.deleteUploadedContent(
              req,
              res,
              path + videoName + '_4096p.mp4',
            );
          }
          if (obj['2048p'] == 1) {
            exports.deleteUploadedContent(
    ...
    if (image) exports.deleteUploadedContent(req, res, image, type);
  }
};
...
exports.getUploadVideoInfo = async url => {
  ...
  if (
    videoUrlObject &&
    videoUrlObject.provider == 'youtube' &&
    videoUrlObject.mediaType == 'video'
  ) {
    responseObject = videoUrlObject;
  } else if (
    videoUrlObject &&
    videoUrlObject.provider == 'vimeo' &&
    videoUrlObject.mediaType == 'video'
  ) {
    responseObject = videoUrlObject;
  } else if (
    videoUrlObject &&
    videoUrlObject.provider == 'dailymotion' &&
    videoUrlObject.mediaType == 'video'
  ) {
    responseObject = videoUrlObject;
  } else if (
    videoUrlObject &&
    videoUrlObject.provider == 'twitch' &&
    (videoUrlObject.mediaType == 'stream' ||
      videoUrlObject.mediaType == 'clip' ||
      videoUrlObject.mediaType == 'video')
  ) {
    responseObject = videoUrlObject;
  } else if (
    /(^http(?:s?):\/\/(?:www\.|web\.|m\.)?facebook\.com\/([A-z0-9\.]+)\/videos(?:\/[0-9A-z].+)?\/(\d+)(?:.+)?$)/.test(
      url,
    )
  ) {
...
exports.getTwitchData = (params, callback) => {
  ...
  if (
    ...
    options.headers.Authorization.indexOf('OAuth') === -1
  ) {
...
exports.getVideoData = async (type, code, req, resObject) => {
  return new Promise(async (resolve, reject) => {
    ...
    switch (type) {
      case 'youtube':
        if (
          req.appSettings['enable_youtube_import'] == '1' &&
          req.appSettings['youtube_api_key'] != ''
        ) {
          url =
            ...
            .then(async result => {
              if (result) {
                ...
                await exports
                  .convertTime(
                    exports
                      .convert_time(data['contentDetails']['duration'])
                      .toString(),
                  )
                  .then(result => {
                ...
                if (
                  ...
                  responseData['image'] =
                    'https://i.ytimg.com/vi/' + code + '/sddefault.jpg';
                responseData['image'] = responseData['image'].replace(
                  'http://',
                  'https://',
                );
        ...
        exports
          ...
          .then(async data => {
            if (data) {
              ...
              await exports
                .convertTime(jsonObj['duration'].toString())
                .then(result => {
                  responseData['duration'] = result;
                });
      ...
      case 'dailymotion':
        url =
          ...
          code +
          '?fields=allow_embed,description,duration,thumbnail_url,title,tags';
        exports
          .getScript(url)
          .then(async result => {
            if (result) {
              ...
              await exports
                .convertTime(parseResponse['duration'].toString())
                .then(result => {
                  responseData['duration'] = result;
                });
      ...
      case 'twitch':
        if (
          req.appSettings['enable_twitch_import'] == '1' &&
          req.appSettings['twitch_api_key'] != ''
        ) {
          ...
          if (resObject.mediaType == 'video') {
            const url =
              (await exports.twitch(resObject.mediaType)) + resObject.id;
            await exports.getTwitchData(
              { clientID: clientID, url: url },
              async (err, res) => {
                try {
                  if (err) {
                    resolve(false);
                    ...
                    responseData['image'] =
                      res['thumbnails']['large'][0]['url'];
                    responseData['image'] = responseData['image'].replace(
                      'http://',
                      'https://',
                    );
                    await exports
                      .convertTime(res['length'].toString())
                      .then(result => {
                        responseData['duration'] = result;
                      });
                    resolve(responseData);
              ...
              async (err, res) => {
                try {
                  if (err) {
                    ...
                    responseData['title'] = res['title'];
                    await exports
                      .convertTime(res['duration'].toString())
                      .then(result => {
                        responseData['duration'] = result;
                      });
              ...
              async (err, res) => {
                try {
                  if (err) {
                    ...
                    const url =
                      (await exports.twitch(resObject.mediaType)) + userId;
                    await exports.getTwitchData(
                      { clientID: clientID, url: url },
                      (err, res) => {
                        try {
                          if (err) {
                            ...
                            responseData['image'] = responseData[
                              'image'
                            ].replace('http://', 'https://');
      ...
      case 'facebook':
        if (
          req.appSettings['enable_facebook_import'] == '1' &&
          req.appSettings['facebook_client_id'] != '' &&
          req.appSettings['facebook_client_secret'] != ''
        ) {
            ...
            .then(async result => {
              if (result) {
                ...
                if (token['access_token']) {
                  ...
                  await exports
                    .getScript(
                      `https://graph.facebook.com/${videoId}?access_token=${token['access_token']}&fields=format,source,description,length,title,tags`,
                    )
                    .then(async results => {
                      if (results) {
                        ...
                        if (res['title']) responseData['title'] = res['title'];
                        if (res['description'])
                          responseData['description'] = res['description'];
                        ...
                        if (res['tags']) responseData['tags'] = res.tags;
                        resolve(responseData);
                      } else {
                        resolve(false);
                      }
          ...
          if (parseResponse.links) {
            if (parseResponse.links.player) {
              ...
              if (parseResponse.meta['duration'])
                await exports
                  .convertTime(parseResponse.meta['duration'].toString())
                  .then(result => {
                    responseData['duration'] = result;
                  });
