...
router.get(
  '/videos/artists/gallery/:artist_id/:page?',
  is_admin,
  controller.getGalleries,
);
router.get(
  '/videos/artists/gallery/create/:artist_id/:id?',
  is_admin,
  controller.createGallery,
);
router.get(
  '/videos/artists/gallery/delete/:id',
  is_admin,
...
router.post(
  ...
  async (req, res, next) => {
    ...
    req.imageResize = [{ width: 800, height: 800 }];
    ...
    currUpload(req, res, function (err) {
      if (err) {
        ...
        if (
          ...
          const pathName =
            req.serverDirectoryPath + '/public/upload/images/artists/gallery/';
    ...
    currUpload(req, res, function (err) {
      if (err) {
        ...
        if (
          ...
          const file = path.basename(req.fileName, extension);
          const pathName =
            req.serverDirectoryPath + '/public/upload/images/videos/artists/';
          const newFileName = file + '_main' + extension;
          var resizeObj = new resize(pathName, req.fileName, req);
          resizeObj
            .save(newFileName, {
...
router.get('/videos/swipe/:page?', is_admin, controller.swipeVideo);

router.get(
  '/videos/sold-video/delete/:id',
  is_admin,
  controller.deleteSoldVideo,
);
...
router.post(
  '/videos/import/create',
  multer().none(),
  is_admin,
  controller.createImportedVideos,
);

router.get(
  '/videos/import/dailymotion/:page?',
  is_admin,
  controller.dailyMotionImport,
);
...
router.post(
  '/videos/categories/change-order',
  is_admin,
  controller.changeOrder,
);

router.get(
  '/videos/categories/delete/:category_id',
  is_admin,
  controller.deleteCategories,
);
router.use(
  '/videos/categories/add/:category_id?',
  is_admin,
  async (req, res, next) => {
    req.allowedFileTypes = /jpeg|jpg|png|gif/;
    req.imageResize = [{ width: req.widthResize, height: req.heightResize }];
    var currUpload = upload(
      'thumbnail',
      'upload/images/categories/videos/',
      req,
    );
    currUpload(req, res, function (err) {
      if (err) {
        ...
        if (
          ...
          const pathName =
            ...
            '/public/upload/images/categories/videos/';
          const newFileName = file + '_main' + extension;
          var resizeObj = new resize(pathName, req.fileName, req);
          resizeObj
            .save(newFileName, {
