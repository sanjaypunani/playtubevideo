  ...
  async (req, res, next) => {
    ...
    if (
      ...
      req.imageResize = [
        {
          suffix: 'res',
          width: req.coverWidthResize,
          height: req.coverHeightResize,
        },
        { suffix: 'original' },
      ];
    }
    ...
    currUpload(req, res, function (err) {
      if (err) {
        ...
        if (
          ...
          const pathName =
            req.serverDirectoryPath + '/public/upload/images/cover/members/';
    ...
    currUpload(req, res, function (err) {
      if (err) {
        ...
        if (
          ...
          const file = path.basename(req.fileName, extension);
          const pathName =
            req.serverDirectoryPath + '/public/upload/images/members/';
          const newFileName = file + '_main' + extension;
          var resizeObj = new resize(pathName, req.fileName, req);
          resizeObj
            .save(newFileName, {
  ...
  [
    check('email')
      ...
      .custom((value, { req }) => {
        // return true;
        if (
          req.item.email != value &&
          typeof req.body.first_name == 'undefined'
        ) {
          return globalModel
            .custom(
              req,
              'SELECT users.user_id FROM users LEFT JOIN userdetails ON userdetails.user_id = users.user_id WHERE users.user_id != ? AND email = ?',
        ...
        if (req.item.username != value) {
          if (req.levelPermissions['member.username'] == 1) {
            ...
            return globalModel
              .custom(
                req,
                'SELECT users.user_id FROM users LEFT JOIN userdetails ON userdetails.user_id = users.user_id WHERE users.user_id != ? AND username = ?',
                [req.body.user_id, value],
              )
              .then(result => {
                if (result && result.length > 0) {
...
router.post(
  ...
  async (req, res, next) => {
    req.allowedFileTypes = /jpeg|jpg|png|gif/;
    req.imageResize = [{ width: 1000, height: 1000 }];
    var currUpload = upload(
      'image',
      'upload/images/members/verifications/',
      req,
    );
    currUpload(req, res, function (err) {
      if (err) {
        ...
        if (
          ...
          const pathName =
            ...
            '/public/upload/images/members/verifications/';
          const newFileName = file + '_main' + extension;
          var resizeObj = new resize(pathName, req.fileName, req);
          resizeObj.save(newFileName).then(res => {
            if (res) {
              fs.unlink(pathName + req.fileName, function (err) {
                if (err) {
                  console.log(err);
...
router.post('/members/blogs', multer().none(), controller.getBlogs);
router.post(
  '/members/insert-dummy-members',
  multer().none(),
  controller.insertDummyMembers,
);
