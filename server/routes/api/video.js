...
router.get('/video/swipeto/', controller.swipeTo);
router.post(
  '/videos/delete',
  isLogin,
  multer().none(),
...
router.post(
    ...
    currUpload(req, res, function (err) {
      if (err) {
        ...
        if (
          ...
          const pathName =
            req.serverDirectoryPath + '/public/upload/images/videos/video/';
  ...
  [
    check('title')
      .not()
      .isEmpty()
      .withMessage(constant.error.TITLEMESSAGE)
      .trim(),
  ],
  controller.create,
);
...
router.post(
  ...
  (req, res, next) => {
    ...
    currUpload(req, res, function (err) {
      if (err) {
        req.imageError =
          'Uploaded video is too large to upload, please choose smaller video and try again.';
        next();
      } else {
        req.fileName = req.file ? req.file.filename : false;
        next();
