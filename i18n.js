...
function makeConfig() {
  ...
  const config = {
    translation: {
      ...
      backend: {
        loadPath: `${
          typeof window == 'undefined' ? process.env.PUBLIC_URL : ''
        }/${LOCALE_PATH}/${LOCALE_STRUCTURE}.json`,
...
function getI18n(config) {
  ...
  i18nModule.use(i18nextXHRBackend).use(LngDetector);
    ...
    if (config.translation.localeSubpaths) {
      i18n.on('languageChanged', lng => {
        if (process.browser) {
          ...
          const [href, as] = lngPathCorrection(config, i18n)(
            originalRoute,
            lng,
          );
          if (as !== originalRoute) {
