...
class Browse extends React.Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    if (prevState.localUpdate) {
      ...
      nextProps.pageData &&
      nextProps.channel_members &&
      nextProps.channel_members != prevState.members
    ) {
      return {
        members: nextProps.channel_members,
        pagging: nextProps.channel_members,
        page: 2,
        search: nextProps.search ? nextProps.search : [],
      };
    } else if (
      nextProps.pageData &&
      nextProps.pageData.members &&
      nextProps.pageData.members != prevState.members
    ) {
      return {
        members: nextProps.pageData.members,
        pagging: nextProps.pageData.pagging,
        page: 2,
        search: nextProps.search ? nextProps.search : [],
      };
    } else if (
      nextProps.pageInfoData.members &&
      nextProps.pageInfoData.members != prevState.members
    ) {
      return {
        members: nextProps.pageInfoData.members,
        pagging: nextProps.pageInfoData.pagging,
        page: 2,
        search: nextProps.search ? nextProps.search : [],
      };
    } else {
  ...
  loadMoreContent(values) {
    ...
    if (this.props.pageInfoData.search) {
      queryString = Object.keys(this.props.pageInfoData.search)
        .map(key => key + '=' + this.props.pageInfoData.search[key])
        .join('&');
      url = `${url}?${queryString}`;
    } else if (this.props.contentType) {
      url = `/dashboard/members/${this.props.contentType}`;
    } else if (this.props.globalSearch) {
      queryString = Object.keys(this.state.search)
      ...
      .then(response => {
        if (response.data.members) {
          ...
          this.setState({
            ...
            pagging: pagging,
            members: [...this.state.members, ...response.data.members],
            loading: false,
          });
        } else {
  ...
  render() {
    let members = this.state.members.map(item => {
      return (
        <div key={item.user_id} className="col-lg-3 col-md-4 col-sm-6 ">
    ...
    return (
      <React.Fragment>
            ...
            <div className="col-sm-12">
              <div className="titleWrap">
                <span className="title">
                  <React.Fragment>
                    {this.props.headerTitle ? this.props.headerTitle : null}
        ...
        <div style={{ padding: '2rem' }}>
          <b>Targets:</b> {`${this.state.members.length} / 3cr`}
        </div>
            ...
            <InfiniteScroll
              ...
              loader={
                <LoadMore
                  ...
                  loading={true}
                  itemCount={this.state.members.length}
                />
              }
              endMessage={
                <EndContent
              ...
              pullDownToRefreshContent={
                <Release release={false} {...this.props} />
              }
              releaseToRefreshContent={
                <Release release={true} {...this.props} />
              }
              refreshFunction={this.refreshContent}
            >
              <div className="container">
                <div className="row mob2col">{members}</div>
              </div>
            </InfiniteScroll>
          </div>
        ) : (
