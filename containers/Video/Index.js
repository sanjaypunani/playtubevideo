...
class Index extends Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    if (prevState.localUpdate) {
      ...
      nextProps.pageInfoData.video != prevState.video ||
      (prevState.video &&
        nextProps.pageInfoData.video.status != prevState.video.status) ||
      nextProps.pageInfoData.password != prevState.password ||
      nextProps.pageInfoData.adultVideo != prevState.adult
    ) {
      return {
  ...
  componentWillUnmount() {
    ...
    if (
      this.props.pageInfoData.appSettings['video_miniplayer'] == 1 &&
      this.props.pageInfoData.appSettings['enable_iframely'] == 0 &&
      this.state.video &&
      ...
      if (this.state.playlistVideos) {
        this.props.updatePlayerData(
          ...
          deleteMessage,
          deleteTitle,
          this.props.pageInfoData.liveStreamingURL,
        );
  ...
  componentDidMount() {
    ...
    userChannel.onmessage = channelData => {
      if (
        channelData.payload &&
        channelData.payload.type &&
        channelData.payload.type == 'LOGOUT'
      ) {
        this.setState({ localUpdate: true, logout: true });
      } else if (
        channelData.data &&
        channelData.data.data &&
        channelData.data.data.payload &&
        channelData.data.data.payload.type == 'LOGOUT'
      ) {
        this.setState({ localUpdate: true, logout: true });
      }
    };
    ...
    if (this.state.video && this.state.video.videoPaymentStatus) {
      if (this.state.video.videoPaymentStatus == 'success') {
        swal(
          'Success',
          Translate(this.props, 'Video purchased successfully.', 'success'),
        );
        ...
        swal(
          'Error',
          Translate(this.props, 'You have cancelled the payment.', 'error'),
        );
      }
    }
    ...
    this.props.socket.on('unwatchlater', data => {
      ...
      if (this.state.video && this.state.video.video_id == id) {
        ...
        if (
          ...
          this.props.pageInfoData.loggedInUserDetails.user_id == ownerId
        ) {
    ...
    this.props.socket.on('unfollowUser', data => {
      ...
      if (
        this.state.video &&
        id == this.state.video.owner.user_id &&
        type == 'members'
      ) {
        if (
          this.props.pageInfoData.loggedInUserDetails &&
    ...
    this.props.socket.on('likeDislike', data => {
      ...
      if (
        this.state.video &&
        itemType == 'videos' &&
        this.state.video.video_id == itemId
      ) {
    ...
    axios
      ...
      .then(response => {
        if (response.data.error) {
          ...
          Router.push(
            `/watch?videoId=${this.props.pageInfoData.videoId}`,
            `/watch/${this.props.pageInfoData.videoId}`,
          );
        }
      })
      .catch(err => {
          ...
          .then(response => {
            if (response.data.error) {
              ...
              this.props.openToast(
                Translate(this.props, response.data.message),
                'success',
              );
              this.setState({ localUpdate: true, logout: true }, () => {
  ...
  fullWidth = e => {
    e.preventDefault();
    this.setState(
      { localUpdate: true, fullWidth: !this.state.fullWidth },
      () => {
        this.getHeight();
      },
    );
  ...
  donationFunction = () => {
    window.location.href = `https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=${
      this.state.video.paypal_email
    }&lc=US&item_name=${
      `Donation+to+` + encodeURI(this.state.video.displayname)
    }&no_note=0&cn=&currency_code=${
      this.props.pageInfoData.appSettings['payment_default_currency']
  ...
  videoEnd = () => {
    ...
    if (this.state.playlistVideos && this.state.playlistVideos.length) {
      ...
      if (itemIndex > -1) {
        ...
        Router.push(
          `/watch?videoId=${items[itemIndex]['custom_url']}&list=${this.state.playlist.custom_url}`,
          `/watch/${items[itemIndex]['custom_url']}?list=${this.state.playlist.custom_url}`,
        );
      }
      ...
      if (
        isAutoplay &&
        this.props.pageInfoData.appSettings['video_autoplay'] == 1 &&
        this.props.pageInfoData.appSettings['enable_iframely'] == 0
      ) {
        ...
        if (this.state.relatedVideos && this.state.relatedVideos.length) {
          ...
          Router.push(
            `/watch?videoId=${items[0]['custom_url']}`,
            `/watch/${items[0]['custom_url']}`,
          );
        }
      }
    }
  ...
  render() {
    ...
    if (this.state.playlistVideos) {
      currentPlaying = this.state.playlistVideos.findIndex(
        p => p['video_id'] == this.state.video.video_id,
      );
        ...
        <div className="container">
          <div className="row">
            {this.state.adult ? (
              ...
              <React.Fragment>
                ...
                <div
                  ...
                  style={{ display: this.state.fullWidth ? 'block' : 'none' }}
                ></div>
                <div
                  className={`${
                    this.state.fullWidth ? 'col-lg-12' : 'col-xl-9 col-lg-8'
                  } videoPlayerHeight`}
                >
                  <div
                  ...
                  >
                    <div className="videoPlayer">
                      <React.Fragment>
                        ...
                        this.state.video.type == 11 ? (
                          <MediaStreaming
                            {...this.props}
                            viewer={this.state.video.total_viewer}
                            height={this.state.width > 992 ? '500px' : '500px'}
                            custom_url={this.state.video.custom_url}
                            streamingId={this.state.video.mediaserver_stream_id}
                            currentTime={this.props.pageInfoData.currentTime}
                            role="audience"
                            imageSuffix={this.props.pageInfoData.imageSuffix}
                            video={this.props.pageInfoData.video}
                            {...this.props.pageInfoData.video}
                          />
                        ) : this.state.video.is_livestreaming ? (
                          <StartLiveStreaming
                            {...this.props}
                            ...
                            height={this.state.width > 992 ? '500px' : '500px'}
                            custom_url={this.state.video.custom_url}
                            channel={this.state.video.channel_name}
                            currentTime={this.props.pageInfoData.currentTime}
                            role="audience"
                            imageSuffix={this.props.pageInfoData.imageSuffix}
                            // video={this.props.pageInfoData.video}
                          ...
                          <Player
                            ...
                            ended={this.videoEnd}
                            height={this.state.width > 992 ? '500px' : '500px'}
                            userAdVideo={this.state.userAdVideo}
                            adminAdVideo={this.state.adminAdVideo}
                    ...
                    {this.state.width > 992 ? (
                      <div
                        className="expand"
                        onClick={this.fullWidth.bind(this)}
                      >
                ...
                this.state.video &&
                this.state.video.approve == 1 &&
                this.state.video.enable_chat == 1 &&
                this.state.video.is_livestreaming == 1 &&
                (this.state.video.channel_name ||
                  this.state.video.mediaserver_stream_id) ? (
                  <div className="col-lg-8 col-xl-9">
                    <div className="ls_sidbar top_video_chat">
                      <Chat
                        {...this.props}
                        channel={this.state.video.channel_name}
                        streamId={this.state.video.mediaserver_stream_id}
                        custom_url={this.state.video.custom_url}
                        comments={
                          this.state.video.chatcomments
                            ? this.state.video.chatcomments
                            : []
                        }
                      />
                ...
                <div className="col-lg-8 col-xl-9">
                  <div className="videoDetailsWrap-content">
                    <div className="videoNameRow">
                      ...
                      <div className="languageSelectorRow">
                        ...
                        <div class="dropdown">
                          <button
                            ...
                            aria-expanded="false"
                          >
                            {this.props.pageInfoData?.currentLanguageVideo
                              ?.language || 'Hindi'}
                          ...
                          >
                            {this.props.pageInfoData.allLanguagesVideo?.map?.(
                              item => {
                                return (
                                  <div
                    ...
                    <div className="videoDetailsLikeWatch">
                      <div className="watchBox">
                        <span title={Translate(this.props, 'Views')}>
                          {this.state.video.view_count + ' '}{' '}
                          {this.props.t('view_count', {
                            count: this.state.video.view_count
                              ? this.state.video.view_count
                              : 0,
                          })}{' '}
                        </span>
                          ...
                          <ul className="LikeDislikeList">
                              ...
                              <React.Fragment>
                                <li>
                                  <Like
                                    icon={true}
                                    {...this.props}
                                    like_count={this.state.video.like_count}
                                ...
                                <li>
                                  <Dislike
                                    ...
                                    {...this.props}
                                    dislike_count={
                                      this.state.video.dislike_count
                                    }
                                    item={this.state.video}
                                    type="video"
                                    id={this.state.video.video_id}
                                ...
                                <li>
                                  <Favourite
                                    ...
                                    {...this.props}
                                    favourite_count={
                                      this.state.video.favourite_count
                                    }
                                    item={this.state.video}
                                    type="video"
                                    id={this.state.video.video_id}
                            ...
                            {this.state.video.approve == 1 ? (
                              <SocialShare
                                ...
                                url={`/watch/${this.state.video.custom_url}`}
                                title={this.state.video.title}
                                imageSuffix={
                                  this.props.pageInfoData.imageSuffix
                                }
                                media={this.state.video.image}
                              />
                            ) : null}
                            <li>
                                ...
                                <ul className="dropdown-menu dropdown-menu-right edit-options">
                                  {this.state.video.canEdit ? (
                                    <li>
                                      <Link
                                        href="/create-video"
                                        customParam={`videoId=${this.state.video.custom_url}`}
                                        as={`/create-video/${this.state.video.custom_url}`}
                                      >
                                        <a
                                          href={`/create-video/${this.state.video.custom_url}`}
                                        >
                                          <span className="material-icons">
                                            edit
                                          </span>
                    ...
                    {this.state.embed ? (
                      <div class="videoEmbed">
                        <textarea
                          name="embed"
                          class="form-control"
                          value={`<iframe src="${config.app_server}/embed/${this.state.video.custom_url}" frameborder="0" width="700" height="400" allowfullscreen><iframe>`}
                        ></textarea>
                      </div>
                    ) : null}

                    {this.props.pageInfoData.appSettings['video_tip'] == 1 &&
                    this.state.video &&
                    this.state.video.tips ? (
                      <Donation
                        {...this.props}
                        item={this.state.video}
                        item_id={this.state.video.video_id}
                        item_type="video"
                      />
                    ) : null}
                      ...
                      <div className="userInfoSubs">
                        <div className="UserInfo">
                          <div className="img">
                            <Link
                              href="/member"
                              customParam={`memberId=${this.state.video.owner.username}`}
                              as={`/${this.state.video.owner.username}`}
                            >
                              <a href={`/${this.state.video.owner.username}`}>
                                <Image
                                  title={this.state.video.owner.displayname}
                                  image={this.state.video.owner.avtar}
                                  imageSuffix={
                                    this.props.pageInfoData.imageSuffix
                                  }
                                />
                          ...
                          <div className="content">
                            ...
                            >
                              <a
                                className="UserName"
                                href={`/${this.state.video.owner.username}`}
                              >
                                <React.Fragment>
                                  {this.state.video.owner.displayname}
                                  {this.props.pageInfoData.appSettings[
                                    'member_verification'
                                  ] == 1 && this.state.video.owner.verified ? (
                                    <span
                            ...
                            <span>
                              <Timeago {...this.props}>
                                {this.state.video.creation_date}
                              </Timeago>
                            </span>
                          </div>
                        </div>
                        <div className="userSubs">
                          <MemberFollow
                            {...this.props}
                            type="members"
                            user={this.state.video.owner}
                            user_id={this.state.video.owner.follower_id}
                          />
                        </div>
                      </div>
                    </div>
            ...
            <div
              ...
              style={{
                marginTop:
                  !this.state.fullWidth && !this.state.adult
                    ? this.state.height
                    : '0px',
              }}
            >
                    ...
                    <p>
                      {
                        <CensorWord
                          {...this.props}
                          text={this.state.playlist.title}
                        />
                      }
                    </p>
                    <p>
                      <Link
                        href="/member"
                        customParam={`memberId=${this.state.playlist.owner.username}`}
                        as={`/${this.state.playlist.owner.username}`}
                      >
                        <a>{this.state.playlist.owner.displayname}</a>
                      </Link>
                      {' - ' +
                        currentPlaying +
                        ' / ' +
                        this.state.playlistVideos.length}
                    </p>
                  </div>
                          ...
                          >
                            ...
                            <div className="sidevideoWrap">
                              <div className="videoImg">
                                <Link
                                  href="/watch"
                                  customParam={`videoId=${video.custom_url}&list=${this.state.playlist.custom_url}`}
                                  as={`/watch/${video.custom_url}?list=${this.state.playlist.custom_url}`}
                                >
                                  <a>
                                    <Image
                                      title={video.title}
              ...
              {this.props.pageInfoData.appSettings['sidebar_video'] ? (
                <AdsIndex
                  ...
                  ads={this.props.pageInfoData.appSettings['sidebar_video']}
                />
              ) : null}
              {this.state.relatedVideos &&
              this.state.relatedVideos.length > 0 ? (
                <RelatedVideos
                  {...this.props}
                  playlist={this.state.playlistVideos}
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    openPlaylist: (open, video_id) =>
      dispatch(playlist.openPlaylist(open, video_id)),
    openToast: (message, typeMessage) =>
      dispatch(playlist.openToast(message, typeMessage)),
    openReport: (status, contentId, contentType) =>
      dispatch(playlist.openReport(status, contentId, contentType)),
    updatePlayerData: (
      relatedVideos,
      playlistVideos,
      currentVideo,
