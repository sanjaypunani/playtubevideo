...
class Video extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      videoTitle: props.pageInfoData.editItem
        ? props.pageInfoData.editItem.title
        : null,
      videoDescription: props.pageInfoData.editItem
        ? props.pageInfoData.editItem.description
        : null,
      tips:
        props.pageInfoData.editItem && props.pageInfoData.editItem.tips
          ? [...props.pageInfoData.editItem.tips]
          : [{ amount: 0 }],
      previousTips:
        props.pageInfoData.editItem && props.pageInfoData.editItem.tips
          ? [...props.pageInfoData.editItem.tips]
          : [{ amount: 0 }],
      videoTags:
        props.pageInfoData.editItem && props.pageInfoData.editItem.tags
          ? props.pageInfoData.editItem.tags.split(',')
          : null,
      videoImage: null,
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    if (
      ...
      nextProps.i18n.language != $('html').attr('lang')
    ) {
      return null;
    }
    if (prevState.localUpdate) {
      ...
      return {
        ...
        openAddTip: false,
        tips:
          nextProps.pageInfoData.editItem &&
          nextProps.pageInfoData.editItem.tips
            ? [...nextProps.pageInfoData.editItem.tips]
            : [{ amount: 0 }],
        previousTips:
          nextProps.pageInfoData.editItem &&
        ...
        videoTitle: nextProps.pageInfoData.editItem
          ? nextProps.pageInfoData.editItem.title
          : null,
        videoDescription: nextProps.pageInfoData.editItem
          ? nextProps.pageInfoData.editItem.description
          : null,
        videoTags:
          nextProps.pageInfoData.editItem &&
          nextProps.pageInfoData.editItem.tags
            ? nextProps.pageInfoData.editItem.tags.split(',')
            : null,
        videoImage: null,
  ...
  componentDidMount() {
    ...
    $(document).on('click', '.add_tips', function () {
      //
      if (
        _this.props.pageInfoData &&
        !_this.props.pageInfoData.loggedInUserDetails
      ) {
  ...
  onSubmit = model => {
    ...
    if (model['duration']) {
      ...
      if (!valid) {
        this.setState({
          localUpdate: true,
          error: [
            { message: this.props.t('Duration must be in format HH:MM:SS.') },
          ],
        });
        return;
      }
    ...
    if (
      this.props.pageInfoData.appSettings['video_commission_type'] == 1 &&
      this.props.pageInfoData.appSettings['video_commission_value'] > 0
    ) {
      if (model['price'] && parseFloat(model['price']) > 0) {
        if (
          model['price'] <=
          this.props.pageInfoData.appSettings['video_commission_value']
        ) {
          let perprice = {};
          perprice['package'] = {
            price:
            ...
            error: [
              {
                message: this.props.t(
                  ...
                  {
                    price: ReactDOMServer.renderToStaticMarkup(
                      <Currency {...this.props} {...perprice} />,
                    ),
                  },
                ),
              },
    ...
    // Append Parent Video
    if (this.state.has_parent) {
      if (this.state.parent_video) {
        formData.append('parent_video', this.state.parent_video);
      }
    }
      ...
      .then(response => {
        if (response.data.error) {
          ...
          if (this.props.chooseVideos) {
            ...
            Router.push(
              `/watch?videoId=${response.data.custom_url}`,
              `/watch/${response.data.custom_url}`,
            );
          }
        }
      })
  ...
  onChangeParentVideo = data => {
    let videoData = this.state.videoSuggections?.find(
      item => item?.id === data?.id,
    );
    this.setState({
      parent_video_data: videoData.video,
      localUpdate: true,
    });
  };

  fetchVideoSuggections = value => {
    const formData = new FormData();
    formData.append('page', 1);
    formData.append('limit', 21);
    axios
      .post(`/search/video?h=${value}&sort=view`, formData)
      .then(response => {
        const suggections = response?.data?.videos?.map(item => {
          return {
            title: item?.title,
            image: item?.orgImage,
            id: item?.video_id,
  ...
  uploadMedia = e => {
    ...
    if (allowedExtensions.indexOf(extension) === -1) {
      alert(
        this.props.t('Invalid file Format. Only {{data}} are allowed.', {
          data: allowedExtensions.join(', '),
        }),
      );
      return false;
    } else if (
      this.props.pageInfoData.appSettings['video_upload_limit'] == 1 &&
      e.size >
        parseInt(this.props.pageInfoData.appSettings['video_upload_limit']) *
          1000000
    ) {
      alert(
        this.props.t('Maximum upload limit is {{upload_limit}}', {
          upload_limit: this.formatBytes(
            parseInt(
              this.props.pageInfoData.appSettings['video_upload_limit'],
            ) * 1000000,
          ),
        }),
      );
  ...
  onSubmitUploadImport = model => {
    ...
    if (key == 'upload' || key == 'import-language') {
      config = {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
        ...
        if (response.data.error) {
          ...
          if (key == 'import-url') {
            ...
            this.setState({
              ...
              success: true,
              videoTitle: response.data.name,
              videoImage: response.data.images[0],
            });
          }
        }
      })
  ...
  removeTip = (id, e) => {
    e.preventDefault();
    let tips = [...this.state.tips];
    let removeElements = !this.state.removeElements
      ? []
      : this.state.removeElements;
  ...
  saveTips = e => {
    ...
    let perprice = {};
    perprice['package'] = {
      price: this.props.pageInfoData.appSettings['videotip_commission_value'],
    };
    tips.forEach((item, index) => {
      if (parseFloat(item.amount) > 0) {
        if (
          parseFloat(
            this.props.pageInfoData.appSettings['videotip_commission_value'],
          ) > 0 &&
          parseInt(
            this.props.pageInfoData.appSettings['videotip_commission_type'],
          ) == 1 &&
        ...
        item.error = this.props.t(
          'Enter amount must be greater than {{price}}.',
          {
            price:
              parseFloat(
                this.props.pageInfoData.appSettings[
                  'videotip_commission_value'
                ],
              ) > 0 &&
              parseInt(
                this.props.pageInfoData.appSettings['videotip_commission_type'],
  ...
  render() {
    ...
    if (this.state.editItem && this.state.editItem.image) {
      if (
        this.state.editItem.image.indexOf('http://') == 0 ||
        this.state.editItem.image.indexOf('https://') == 0
      ) {
        imageUrl = this.state.editItem.image;
      } else {
        if (
          this.props.pageInfoData.livestreamingtype == 0 &&
          this.state.editItem.mediaserver_stream_id &&
          this.state.editItem.image &&
          this.state.editItem.image.indexOf('LiveApp/previews') > 0
        ) {
          imageUrl =
            this.props.pageInfoData.liveStreamingServerURL +
            ':5443' +
            this.state.editItem.image;
        } else {
          imageUrl =
            this.props.pageInfoData.imageSuffix + this.state.editItem.image;
        }
      }
    }
    let formFields = [];
      ...
      formFields = [
        {
          key: 'title',
          label: 'Video Title',
          value: this.state.editItem ? this.state.editItem.title : null,
          isRequired: true,
        },
        {
          key: 'description',
          ...
          type: 'textarea',
          value: this.state.editItem ? this.state.editItem.description : null,
        },
      ];
    ...
    if (
      this.state.chooseType == 'embed' ||
      (this.state.editItem && this.state.editItem.type == 22)
    ) {
      formFields.push({
        key: 'duration',
        label: 'Video Duration (eg - HH:MM:SS)',
        value: this.state.editItem ? this.state.editItem.duration : null,
      });
    }

    if (
      this.state.chooseType == 'upload' &&
      this.state.sell_videos &&
      (!this.state.editItem || this.state.editItem.type == 3)
    ) {
      validator.push({
        key: 'price',
        validations: [
      ...
      formFields.push({
        ...
        value: this.state.editItem ? this.state.editItem.price : null,
        isRequired: true,
      });
      if (
        this.props.pageInfoData.appSettings['video_commission_type'] == 1 &&
        ...
        let perprice = {};
        perprice['package'] = {
          price: this.props.pageInfoData.appSettings['video_commission_value'],
        };
        formFields.push({
          key: 'price_desc_1',
          type: 'content',
          content:
            '<span>' +
            this.props.t('Price enter must be greater than {{price}}.', {
              price: ReactDOMServer.renderToStaticMarkup(
                <Currency {...this.props} {...perprice} />,
              ),
            }) +
    ...
    if (
      this.props.pageInfoData.videoCategories &&
      ...
      categories.push({
        key: 0,
        value: 0,
        label: 'Please Select Video Category',
      });
          ...
          if (res.category_id == this.state.category_id) {
            if (res.subcategories) {
              subcategories.push({
                key: 0,
                value: 0,
                label: 'Please Select Video Sub Category',
              });
                    ...
                    if (rescat.category_id == this.state.subcategory_id) {
                      if (rescat.subsubcategories) {
                        subsubcategories.push({
                          key: 0,
                          value: 0,
                          label: 'Please Select Video Sub Sub Category',
                        });
    ...
    languagesArray.push({
      key: 0,
      value: 0,
      label: 'Please Select Video Language',
    });
    languages.forEach(res => {
      languagesArray.push({
        key: res.value,
        label: res.name,
        value: res.value,
    ...
    formFields.push({
      ...
      onChangeFunction: this.onLanguageChange,
    ...
    //   type: 'checkbox',
    //   subtype: 'single',
    //   onChangeFunction: this.onHasParentChange,
    //   value: [this.state.has_parent],
    //   options: [
    //     {
    //       value: '1',
    //       label: 'Link video to parent',
    //       key: 'has_parent_1',
    //     },
    //   ],
    // });
    ...
    if (this.state.chooseType === 'import-language') {
      formFields.push({
        ...
        fetchRequest: this.fetchVideoSuggections,
        // onChangeFunction: this.onChangeParentVideo,
        onSelectVideo: this.onChangeParentVideo,
        placeholder: 'Search for a parent Video...',
        suggestionValue: this.state.videoSuggections,
        label: 'Parent video',
      });
    }
    ...
    if (this.state.chooseType !== 'import-language') {
      ...
      if (this.props.pageInfoData.videoArtists) {
        ...
        this.props.pageInfoData.videoArtists.forEach(res => {
          artists.push({
            ...
            key: res.title,
            label: res.title,
            image: this.props.pageInfoData.imageSuffix + res.image,
          });
        });
        ...
        formFields.push({
          ...
          type: 'checkbox',
          value:
            this.state.editItem && this.state.editItem.artists
              ? this.state.editItem.artists.split(',')
              : null,
          options: artists,
        });
      }
      ...
      formFields.push({
        ...
        subtype: 'single',
        value: this.state.editItem
          ? [this.state.editItem.search ? '1' : '0']
          : ['1'],
        options: [
          {
            value: '1',
      ...
      if (this.props.pageInfoData.appSettings['enable_comment_approve'] == 1) {
        ...
        formFields.push({
          ...
          type: 'select',
          value: this.state.editItem
            ? this.state.editItem.autoapprove_comments.toString()
            : '1',
          options: comments,
        });
      }
      ...
      if (this.state.privacy == 'password') {
        formFields.push({
          ...
          label: 'Password',
          type: 'password',
          value: this.state.editItem ? this.state.editItem.password : '',
          isRequired: true,
        });
        validator.push({
          key: 'password',
    ...
    if (
      ...
      fieldUploadImport.push({
        ...
        onChangeFunction: this.uploadMedia,
      });
            ...
            <div className="comments">
              <div className="VideoDetails-commentWrap tip_cnt">
                <div className="popup_wrapper_cnt_header">
                  ...
                  <a onClick={this.closeTipPopup} className="_close">
                    <i></i>
                  </a>
                </div>
                  ...
                  <div className="row">
                    <form>
                              ...
                              <div className="form-group" key={i}>
                                <div className="tip_input">
                                  <input
                                    ...
                                    onChange={this.setAmount.bind(this, i)}
                                  />
                                  {this.state.tips.length > 1 ? (
                                    <a
                                      href="#"
                                      onClick={this.removeTip.bind(this, i)}
                                      className="remove"
                                    >
                                      {this.props.t('remove')}
                                    </a>
                      ...
                      <div className="form-group">
                        ...
                        <button
                          type="button"
                          onClick={this.saveTips.bind(this)}
                        >
                          {Translate(this.props, 'Save')}
                        </button>
                        <button
                          type="button"
                          className="add_more_tip"
                          onClick={this.addMoreRow.bind(this)}
                        >
                          {Translate(this.props, 'Add More')}
                        </button>
                      </div>
                    </form>
                  </div>
          ...
          !this.state.channel_id ? (
            <React.Fragment>
              <Breadcrum
                {...this.props}
                image={
                  this.props.pageData['pageInfo']['banner']
                    ? this.props.pageData['pageInfo']['banner']
                    : '/static/images/breadcumb-bg.jpg'
                }
                title={`${this.state.editItem ? 'Edit' : 'Create'} Video`}
              />
                ...
                <div className="container">
                  <div className="BtnUpld">
                    {this.state.enableUploadVideo ? (
                      <a
                        href="#"
                        onClick={this.chooseType.bind(this, 'upload')}
                        className={
                          this.state.chooseType == 'upload' ? 'active' : ''
                        }
                      >
                    ...
                    <a
                      href="#"
                      onClick={this.chooseType.bind(this, 'import')}
                      className={
                        this.state.chooseType == 'import' ? 'active' : ''
                      }
                    >
                    ...
                    <a
                      href="#"
                      onClick={this.chooseType.bind(this, 'import-language')}
                      className={
                        this.state.chooseType == 'import-language'
                          ? 'active'
                          : ''
                    ...
                    >
                      {Translate(this.props, 'Import Language')}
                    </a>
                    {this.props.pageInfoData.levelPermissions[
                      'video.embedcode'
                    ] == 1 ? (
                      <a
                        href="#"
                        onClick={this.chooseType.bind(this, 'embed')}
                        className={
                          this.state.chooseType == 'embed' ? 'active' : ''
                        }
                      >
