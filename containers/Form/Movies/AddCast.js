...
class AddCast extends Component {
  ...
  onSubmit = model => {
    ...
    if (this.state.editItem) {
      formData.append(
        'cast_crew_member_id',
        this.state.editItem.cast_crew_member_id,
      );
  ...
  render() {
      ...
      suggestionValue = [
        {
          ...
          id: this.state.editItem.cast_crew_id,
        },
      ];
    }
    let formFields = [];
        ...
        validations: [
          {
            ...
            message: this.props.isCrew
              ? 'Crew Member is required field'
              : 'Cast Member is required field',
          },
        ],
      });
      formFields.push({
        key: 'cast_crew_member_id',
        type: 'autosuggest',
        id: this.state.editItem ? this.state.editItem.cast_crew_id : '',
        imageSuffix: this.props.pageInfoData.imageSuffix,
        url:
          config.app_server +
          '/api/movies/cast/auto-suggest' +
          (this.props.fromCastnCrew ? '/movie' : ''),
        placeholder: 'Search for a member...',
        suggestionValue: suggestionValue,
        label: 'Cast Member',
        value: this.state.editItem ? this.state.editItem.name : null,
        isRequired: true,
      });
    } else {
      formFields.push({
    ...
    if (!this.props.isCrew) {
      ...
      formFields.push({
        ...
        type: 'text',
        label: 'Character',
        value: this.state.editItem ? this.state.editItem.character : null,
        isRequired: true,
      });
    } else {
      validator.push(
      ...
      formFields.push(
        {
          ...
          type: 'text',
          label: 'Job',
          value: this.state.editItem ? this.state.editItem.job : null,
          isRequired: true,
        },
        {
          key: 'department',
          type: 'text',
          label: 'Department',
          value: this.state.editItem ? this.state.editItem.department : null,
          isRequired: true,
        },
      );
    }
