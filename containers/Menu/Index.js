...
class Menu extends Component {
  constructor(props) {
    super(props);
    this.state = {
      search: props.pageInfoData.searchTitleText
        ? props.pageInfoData.searchTitleText
        : '',
    ...
    this.setCreateButtonsWrapperRef =
      this.setCreateButtonsWrapperRef.bind(this);
  ...
  openToggle = (type, e) => {
    ...
    if (type != this.state.type) style = 'none';
      ...
      this.setState(
        ...
        () => {
          if (this.props.mobileMenu) {
            if (this.state.style == 'block') $('body').addClass('menu_open');
  ...
  handleClickOutside(e) {
    ...
    if (
      ...
      (!$(e.target).data('toggle') ||
        ($(e.target).data('toggle') &&
          $(e.target).data('toggle') != 'dropdown'))
    ) {
      if (this.state.type == 'notifications') {
        if (
          this.dropNotificationRef &&
          !this.dropNotificationRef.contains(e.target)
        ) {
          if (
        ...
        if (this.searchRef && !this.searchRef.contains(e.target)) {
          ...
          this.searchTableRef &&
          !this.searchTableRef.contains(e.target)
        ) {
          style = 'none';
        }
      }
      ...
      (!$(e.target).data('toggle') ||
        ($(e.target).data('toggle') &&
          $(e.target).data('toggle') != 'dropdown')) &&
      (!this.dropMenuButtons || !this.dropMenuButtons.contains(e.target))
    ) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    if (prevState.localUpdate) {
      ...
      nextProps.pageInfoData.searchTitleText &&
      nextProps.pageInfoData.searchTitleText != prevState.search
    ) {
      return {
        search: nextProps.pageInfoData.searchTitleText
          ? nextProps.pageInfoData.searchTitleText
          : '',
        style: false,
      };
  ...
  componentDidMount() {
    ...
    this.props.socket.on('notifications', data => {
      if (
        ...
        if (
          data.owner_id == this.props.pageInfoData.loggedInUserDetails.user_id
        ) {
          this.setState(
            {
              localUpdate: true,
              notifications: [data.notification, ...this.state.notifications],
  ...
  render() {
    if (
      !this.props.pageInfoData ||
      !this.props.pageInfoData.menus ||
      !this.props.pageInfoData.menus.menus
    ) {
      return null;
    }
    let menus = null;
                      ...
                      <ul className="custmenuRight">
                        ...
                        >
                          <a
                            ...
                            onClick={e => this.openToggle('settings', e)}
                            style={{ cursor: 'pointer' }}
                            href="#"
                            role="button"
                          >
                            <span className="material-icons notclose parent">
                              account_circle
                            </span>
                            <span className="material-icons notclose parent">
                              arrow_drop_down
                          ...
                          <ul
                            ...
                            ref={this.props.setSettingsWrapperRef}
                      ...
                      {this.props.pageInfoData.loggedInUserDetails ? (
                        <React.Fragment>
                          <Notifications
                            ...
                            openToggle={this.openToggle}
                            setNotificationWrapperRef={
                              this.setNotificationWrapperRef
                            }
                          />
                          <SettingMenus
                            type="website"
                            ...
                            openToggle={this.openToggle}
                            setSettingsWrapperRef={this.setSettingsWrapperRef}
                          />
                        </React.Fragment>
                      ) : null}
                    </ul>
              ...
              <div className="MobmenuWrap">
                <nav className="navbar navbar-toggleable-lg">
                  ...
                  >
                    {!this.props.pageInfoData.loggedInUserDetails ? (
                      <div
                        className="mobMenuLoginBtn"
                        ref={this.setMenuButtons}
                      >
                        <ul>
                          {!this.props.loginButtonHide ? (
                            this.props.redirectLogin ? (
                    ...
                    {this.props.pageInfoData.appSettings['fixed_header'] ==
                    0 ? (
            ...
            <div className="MobRightSide headerRightMenu">
              <ul className="mobRightNav ">
                ...
                <SettingMenus
                  ...
                  type="mobile"
                  style={
                    this.state.type == 'settings' ? this.state.style : 'none'
                  }
                  openToggle={this.openToggle}
                  setSettingsWrapperRef={this.setSettingsWrapperRef}
                />
                <Search
                  ...
                  type="mobile"
                  style={
                    this.state.type == 'search' ? this.state.style : 'none'
                  }
                  openToggle={this.openToggle}
                  setSettingsWrapperRef={this.setSettingsWrapperRef}
                />
