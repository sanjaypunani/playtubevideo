  ...
  allowAdultContent = e => {
    this.setState({ adult: !this.state.adult }, () => {
      ...
      axios.post(url, formData).then(response => {
        Router.push(
          this.state.previousUrl ? this.state.previousUrl : Router.pathname,
        );
      });
    });
  };
    ...
    if (this.props.pageInfoData.menus && this.props.pageInfoData.menus.menus) {
      menus = this.props.pageInfoData.menus.menus.map(elem => {
        return (
          <li
            onClick={e => {
              this.openMenu(e, elem.submenus);
            }}
            className={
              'nav-item' +
              (elem.submenus ? ' dropdown dropdownmenu' : '') +
              (this.props.menuOpen && elem.submenus ? ' hide' : '')
            }
            key={elem.menu_id}
          >
            <Link
            ...
            >
              <a
                className={
                  ...
                  (path.indexOf(elem.url) > -1 &&
                  path.indexOf(elem.url + '/') < 0
                    ? ' active'
                    : '')
                }
                target={elem.target}
            ...
            {elem.submenus ? (
              <ul
                className={`submenu collapse${
                  this.state.submenu &&
                  this.state.submenu == elem.submenus[0].menu_id
                    ? ' show'
                    : ''
                }`}
              >
                {elem.submenus.map(subMenu => {
                  return (
                    <li
                      onClick={e => {
                        this.openSubMenu(e, subMenu.subsubmenus);
                      }}
                      className={
                        'nav-item' +
    ...
    return (
      <div className={`sidebar-menu${menuOpen ? ' mini-menu' : ''}`}>
        ...
        <div className="menu-list sidebar-scroll">
          <ul
            className={`main-menu border-sidebar${
              menuOpen ? ' small-menu' : ''
            }`}
          >
            <li>
              <Link href="/">
                <a className={path == '/' ? 'active' : ''}>
                  ...
                  {Translate(this.props, 'Home')}
                </a>
              </Link>
            </li>
            {this.props.pageData.levelPermissions &&
            this.props.pageData.levelPermissions['livestreaming.create'] == 1 &&
            this.props.pageData.liveStreamingEnable ? (
              <li>
              ...
              <React.Fragment>
                ...
                <li>
                  ...
                  >
                    <a
                      className={
                        path.indexOf('/dashboard/purchases') > -1
                          ? 'active'
                          : ''
                      }
                    >
            ...
            <li>
              <Link
                href="/videos"
                as="/videos/latest"
                customParam="sort=latest"
              >
                <a
                  className={
                    path.indexOf('/videos/latest') > -1 ? 'active' : ''
                  }
                >
                  <span className="material-icons">videocam</span>{' '}
                  {Translate(this.props, 'Latest Videos')}
                </a>
              </Link>
            </li>
            <li>
              ...
              >
                ...
                >
                  ...
                  {Translate(this.props, 'Trending Videos')}
                </a>
              </Link>
            </li>
            <li>
              <Link href="/videos" as="/videos/top" customParam="pageType=top">
                <a className={path.indexOf('/videos/top') > -1 ? 'active' : ''}>
                  ...
                  {Translate(this.props, 'Top Videos')}
                </a>
              </Link>
            </li>
          </ul>

          {this.props.pageInfoData.channelSubscriptions &&
          this.props.pageInfoData.channelSubscriptions.length > 0 ? (
            <ul
              className={`main-menu border-sidebar${menuOpen ? ' hide' : ''}`}
            >
              <li className={`sidebar-menu-title`}>
                {Translate(this.props, 'Subscriptions')}
              </li>
              {this.props.pageInfoData.channelSubscriptions.map(channel => {
                return (
                  <li key={channel.custom_url}>
                    <Link
                      href="/channel"
                      customParam={`channelId=${channel.custom_url}`}
              ...
              <li>
                <Link href="/channels">
                  <a>
                    ...
                    {Translate(this.props, 'Browse channels')}
                  </a>
                </Link>
              </li>
            </ul>
          ...
          {this.props.pageInfoData.popularMembers ? (
            ...
            >
              <li className="sidebar-menu-title">
                {Translate(this.props, 'Popular Members')}
              </li>
                ...
                if (index < 5) {
                  return (
                    <li key={member.user_id}>
                      <Link
                        href="/member"
                        customParam={`memberId=${member.username}`}
                        as={`/${member.username}`}
                      >
                        <a className="sidebar-icon">
                          <Image
                            ...
                            imageSuffix={this.props.pageInfoData.imageSuffix}
                            image={member.avtar}
                            title={this.props.t(member.displayname)}
                          />
                          {member.displayname}
                        </a>
                      </Link>
          ...
          <div className={`sidebar-menu-two${menuOpen ? ' hide' : ''}`}>
                ...
                {this.props.pageInfoData.categoriesVideo.map(category => {
                  return (
                    <li key={category.category_id}>
                      <Link
                        href={`/category`}
                        customParam={`type=video&categoryId=` + category.slug}
            ...
            <ul className="main-menu border-sidebar">
              ...
              {this.props.pageInfoData.appSettings['video_adult'] == 1 ||
              this.props.pageInfoData.appSettings['channel_adult'] == 1 ||
              this.props.pageInfoData.appSettings['blog_adult'] == 1 ||
              this.props.pageInfoData.appSettings['playlist_adult'] == 1 ? (
                <div className="custom-control custom-switch adultSwitchFtr">
                  <input
                    onChange={this.allowAdultContent}
                    defaultChecked={this.state.adult}
                    type="checkbox"
                  ...
                  <label
                    ...
                    htmlFor="adultSwitchFtr"
                  >
                    {this.props.t('Adult content')}
                  </label>
                ...
                {bottomFooterMenus.map(menu => {
                  return (
                    <Link href={menu.url} key={menu.menu_id}>
                      <a target={menu.target}>{this.props.t(menu.label)}</a>
                    </Link>
                  );
                })}
              </div>
            ...
            <div className="copyright">
              {this.props.t(
                'Copyright © {{year}} {{site_title}}. All Rights Reserved.',
                {
                  ...
                  site_title: this.props.pageInfoData.appSettings['site_title'],
                },
              )}
            </div>
          </div>
